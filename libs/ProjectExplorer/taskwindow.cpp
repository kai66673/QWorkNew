...
public:
    ...
    QAction *m_filterWarningsAction;
    QAction *m_categoriesAction;
    QMenu *m_categoriesMenu;
    QList<QAction *> m_actions;
};

static QAction *createFilterButton(QObject *parent, const QString &title, QIcon icon, const QString &toolTip,
...
//    Core::ICore::addContextObject(d->m_taskWindowContext);
    ...
    d->m_filterWarningsAction = createFilterButton(this, tr("Warnings"),
                                                   QIcon(QLatin1String(Core::Constants::ICON_WARNING)),
                                                   tr("Show Warnings"), this, SLOT(setShowWarnings(bool)));

    d->m_categoriesAction = new QAction(tr("Categories"), this);
    d->m_categoriesAction->setIcon(QIcon(QLatin1String(Core::Constants::ICON_FILTER)));
    d->m_categoriesAction->setToolTip(tr("Filter by categories"));
    d->m_categoriesAction->setProperty("noArrow", true);
//    connect(d->m_categoriesAction, SIGNAL(triggered()), this, SLOT(showFilterCategories()));
...
//    Core::ICore::removeContextObject(d->m_taskWindowContext);
    delete d->m_filterWarningsAction;
    delete d->m_listview;
    delete d->m_filter;
    delete d->m_model;
...
void TaskWindow::delayedInitialization()
{
    ...
    if (ITaskHandler *h = ProjectManager::taskHandler()) {
...
QList<QAction *> TaskWindow::toolBarActions() const
{
    return QList<QAction*>() << d->m_filterWarningsAction << d->m_categoriesAction;
}
...
void TaskWindow::showFilterCategories()
{
    d->m_categoriesMenu->exec(QCursor::pos());
}
