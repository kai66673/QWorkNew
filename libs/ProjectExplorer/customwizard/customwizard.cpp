...
{
    ICustomWizardMetaFactory *factory = 0;
    QList<ICustomWizardMetaFactory *> factories = ProjectManager::customWizardMetaFactories();
    foreach (ICustomWizardMetaFactory *f, factories) {
        if ( p->klass.isEmpty() ? (p->kind == f->kind()) : (p->klass == f->klass()) ) {
            factory = f;
            break;
        }
    }
...
QList<CustomWizard*> CustomWizard::createWizards()
{
    ...
    QString errorMessage;
    QString verboseLog;
    const QString templateDirName = qApp->applicationDirPath() +
                                    QLatin1Char('/') + QLatin1String(templatePathC);


    const QString userTemplateDirName = Utils::FileUtils::applicationDataLocation() +
                                        QLatin1String("/.") + QLatin1String(templatePathC);
    ...
    if (CustomWizardPrivate::verbose) { // Print to output pane for Windows.
        ...
        Core::LogManager::addMessage(Core::LogMessage(verboseLog, "Wzrd"));
    }
    return rc;
}
...
bool CustomProjectWizard::postGenerateOpen(const Core::GeneratedFiles &l, QString *errorMessage)
{
    ...
    foreach (const Core::GeneratedFile &file, l) {
        if (file.attributes() & Core::GeneratedFile::OpenProjectAttribute) {
            if (!ProjectManager::instance()->openProjectFilePath(file.path()))
                return false;
        }
    }
