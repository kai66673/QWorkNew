...
static const bool lowerCaseFilesDefault = true;
...
{
    s->beginGroup(QLatin1String(ProjectExplorer::Constants::CPPTOOLS_SETTINGSGROUP));
    ...
    s->setValue(QLatin1String(ProjectExplorer::Constants::LOWERCASE_CPPFILES_KEY), lowerCaseFiles);
...
void CppFileSettings::fromSettings(QSettings *s)
{
    ...
    sourceSearchPaths = s->value(QLatin1String(sourceSearchPathsKeyC), defaultSourceSearchPaths)
            .toStringList();
    lowerCaseFiles = s->value(QLatin1String(ProjectExplorer::Constants::LOWERCASE_CPPFILES_KEY), QVariant(lowerCaseFilesDefault)).toBool();
...
bool CppFileSettings::applySuffixesToMimeDB()
{
    ...
    return mimeDb->setPreferredSuffix(QLatin1String(ProjectExplorer::Constants::CPP_SOURCE_MIMETYPE), sourceSuffix) &&
           mimeDb->setPreferredSuffix(QLatin1String(ProjectExplorer::Constants::CPP_HEADER_MIMETYPE), headerSuffix);
}
...
QString CppFileSettings::licenseTemplate(const QString &fileName, const QString &className)
{
    ...
    const QSettings *s = ProjectExplorer::ProjectManager::settings();
    QString key = QLatin1String(ProjectExplorer::Constants::CPPTOOLS_SETTINGSGROUP);
    key += QLatin1Char('/');
    ...
    /// TODO: !
//    licenseStream.setCodec(DocumentManager::getInstance()->defaultTextCodec());
    ...
    m_ui(new Internal::Ui::CppFileSettingsPage)
{
    ...
    if (const Core::MimeType sourceMt = mimeDb->findByType(QLatin1String(ProjectExplorer::Constants::CPP_SOURCE_MIMETYPE)))
        foreach (const QString &suffix, sourceMt.suffixes())
            m_ui->sourceSuffixComboBox->addItem(suffix);

    if (const Core::MimeType headerMt = mimeDb->findByType(QLatin1String(ProjectExplorer::Constants::CPP_HEADER_MIMETYPE)))
...
void CppFileSettingsWidget::slotEdit()
{
    ...
    DocumentManager::getInstance()->openDocument(path);
}
...
CppFileSettingsPage::CppFileSettingsPage(QSharedPointer<CppFileSettings> &settings,
    ...
    m_settings(settings)
{
    setId(ProjectExplorer::Constants::CPP_FILE_SETTINGS_ID);
    setDisplayName(QCoreApplication::translate("CppTools", ProjectExplorer::Constants::CPP_FILE_SETTINGS_NAME));
    setCategory(ProjectExplorer::Constants::CPP_SETTINGS_CATEGORY);
    setDisplayCategory(QCoreApplication::translate("CppTools", ProjectExplorer::Constants::CPP_SETTINGS_TR_CATEGORY));
    setCategoryIcon(QLatin1String(ProjectExplorer::Constants::SETTINGS_CATEGORY_CPP_ICON));
}
    ...
    if (m_widget) {
        ...
        if (newSettings != *m_settings) {
            *m_settings = newSettings;
            m_settings->toSettings(ProjectExplorer::ProjectManager::settings());
