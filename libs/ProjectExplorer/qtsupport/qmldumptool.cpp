...
//    return Core::ICore::resourcePath() + QLatin1String("/qml/qmldump/");
    return qApp->applicationDirPath() + QLatin1String("/.qml/qmldump/");
}
                                     ...
                                     bool preferDebug, QString *dumperPath, Utils::Environment *env)
{
    ...
    if (version && !version->hasQmlDump() && QmlDumpTool::canBuild(version)) {
        ...
        if (qmlDumpBuildTask) {
            ...
            Core::Storage::mainWindow()->progressManager()->addTask(task, taskName, "QmakeProjectManager::BuildHelpers");
        }
        return;
    }
