...
{
    QStringList numbers = versionString.split('.');
    if ( numbers.size() != 3 )
        majorVersion = minorVersion = patchVersion = -1;
    else {
        bool isNumber;
        majorVersion = numbers[0].toInt(&isNumber);
        minorVersion = numbers[1].toInt(&isNumber);
        patchVersion = numbers[2].toInt(&isNumber);
        if ( !isNumber )
            majorVersion = minorVersion = patchVersion = -1;
    }
}
...
void BaseQtVersion::buildDebuggingHelper(ToolChain *tc, int tools)
{
    ...
    Core::Storage::mainWindow()->progressManager()->addTask(task, taskName, "Qt::BuildHelpers");
