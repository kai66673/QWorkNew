...
const char QTVERSION_FILENAME[] = "/.settings/qtversion.xml";
...
{
    return FileName::fromString(qApp->applicationDirPath() + QLatin1String(QTVERSION_FILENAME));
}

static FileName settingsFileName(const QString &path)
{
    return FileName::fromString(Utils::FileUtils::applicationDataLocation() + path);
}
...
QtVersionManager::~QtVersionManager()
{
    ...
    m_versions.clear();
    qDeleteAll(m_qtVersionFactories);
}
...
static bool restoreQtVersions()
{
    ...
    QList<QtVersionFactory *> factories = QtVersionManager::qtVersionFactories();
...
static void saveQtVersions()
{
    ...
    m_writer->save(data, Core::Storage::mainWindow()->widget());
}
...
QList<QtVersionFactory *> QtVersionManager::qtVersionFactories()
{ return m_instance->m_qtVersionFactories; }

void QtVersionManager::registerQtVersionFactory(QtVersionFactory *factory)
{ m_instance->m_qtVersionFactories << factory; }
