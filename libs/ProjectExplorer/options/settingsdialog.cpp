...
{
    QList<Core::IOptionsPage*> rc = ProjectExplorer::ProjectManager::optionPages();
    ...
    m_finished(false)
{
    ...
    m_model->setPages(m_pages, QList<IOptionsPageProvider *>());
...
void SettingsDialog::showPage(Id categoryId, Id pageId)
{
    ...
    if (!initialCategory.isValid() && !initialPage.isValid()) {
        QSettings *settings = ProjectExplorer::ProjectManager::settings();
...
void SettingsDialog::done(int val)
{
    ...
    ProjectExplorer::ProjectManager::saveSettings(); // save all settings
