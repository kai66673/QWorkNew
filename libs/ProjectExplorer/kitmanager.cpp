...
const char KIT_FILENAME[] = "/.settings/profiles.xml";

static FileName settingsFileName()
{
    QString settingsLocation = FileUtils::applicationDataLocation();
    return FileName::fromString(settingsLocation + QLatin1String(KIT_FILENAME));
}
    ...
    QObject(parent)
{
    ...
    connect(ProjectManager::instance(), &ProjectManager::saveSettingsRequested,
...
void KitManager::restoreKits()
{
    ...
    QString settingsLocation = qApp->applicationDirPath();
    QFileInfo kitFile(settingsLocation + QLatin1String(KIT_FILENAME));
    if (kitFile.exists()) {
