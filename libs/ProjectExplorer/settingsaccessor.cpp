...
{
    return ProjectManager::projectExplorerSettings().environmentId.toByteArray();
}
...
void UserFileVersion11Upgrader::parseQtversionFile()
{
    ...
    reader.load(Utils::FileName::fromString(settingsLocation + QLatin1String("/qtversion.xml")));
...
void UserFileVersion11Upgrader::parseToolChainFile()
{
    PersistentSettingsReader reader;
    QString settingsLocation = FileUtils::applicationDataLocation();
    reader.load(Utils::FileName::fromString(settingsLocation + QLatin1String("/toolChains.xml")));
    QVariantMap data = reader.restoreValues();
