...
{
    Project *project = 0;
    QList<IProjectProvider *> managerList = ProjectManager::projectProviders();
    ...
    foreach (const JsonWizard::GeneratorFile &f, files) {
        if (f.file.attributes() & Core::GeneratedFile::OpenProjectAttribute) {
            ...
            Core::MimeType mt = Core::Storage::mainWindow()->mimeDatabase()->findByFile(fi);
            if (mt.isNull())
                continue;

            foreach (IProjectProvider *manager, managerList) {
