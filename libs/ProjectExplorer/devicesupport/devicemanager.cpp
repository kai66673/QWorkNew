...
void DeviceManager::save()
{
    ...
    d->writer->save(data, Core::Storage::mainWindow()->widget());
}
...
{
    return Utils::FileName::fromString(Utils::FileUtils::applicationDataLocation() + "/.devices" + extension);
}

Utils::FileName DeviceManager::systemSettingsFilePath(const QString &deviceFileRelativePath)
{
    return Utils::FileName::fromString(
              Utils::FileUtils::applicationDataLocation() + "/.devices"
...
{
    QList<IDeviceFactory *> deviceFactories = ProjectManager::deviceFactories();
    IDeviceFactory *factory = 0;

    foreach ( IDeviceFactory *f, deviceFactories ) {
        if ( f->canRestore(map) ) {
            factory = f;
            break;
        }
...
DeviceManager::DeviceManager(bool isInstance) : d(new DeviceManagerPrivate)
{
    if (isInstance) {
        ...
        m_instance = this;
        connect(ProjectManager::instance(), SIGNAL(saveSettingsRequested()), SLOT(save()));
    }
}
