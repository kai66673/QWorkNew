...
KitInformation::ItemList DeviceTypeKitInformation::toUserOutput(const Kit *k) const
{
    ...
    if (type.isValid()) {
        QList<ProjectExplorer::IDeviceFactory *> deviceFactories = ProjectManager::deviceFactories();
        IDeviceFactory *factory = 0;
        foreach ( ProjectExplorer::IDeviceFactory *f, deviceFactories ) {
            if ( f->availableCreationIds().contains(type) ) {
                factory = f;
                break;
            }
        }
