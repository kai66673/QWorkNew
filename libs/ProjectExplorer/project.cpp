...
{
    return Utils::FileName::fromString(filePath());
}
...
void Project::saveSettings()
{
    ...
    if (!targets().isEmpty())
        d->m_accessor->saveSettings(toMap(), Core::Storage::mainWindow()->widget());
}
...
bool Project::restoreSettings()
{
    if (!d->m_accessor)
        d->m_accessor = new Internal::UserFileAccessor(this);
    QVariantMap map(d->m_accessor->restoreSettings(Core::Storage::mainWindow()->widget()));
    bool ok = fromMap(map);
    if (ok)
        emit settingsLoaded();
    return ok;
...
IProjectProvider *Project::projectProvider() const
{ return m_projectProvider; }

void Project::setProjectProvider(IProjectProvider *projectProvider)
