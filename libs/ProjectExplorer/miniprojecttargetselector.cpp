    ...
    : ListWidget(parent), m_ignoreIndexChange(false)
{
    ...
    connect(projectManager, SIGNAL(aboutToRemoveProject(ProjectExplorer::Project*)),
    ...
    connect(projectManager, SIGNAL(projectDisplayNameChanged(ProjectExplorer::Project*)),
...
void ProjectListWidget::setProject(int index)
{
    ...
    ProjectManager::setStartupProject(p);
}
...
MiniProjectTargetSelector::MiniProjectTargetSelector(QToolButton *btn, QWidget *parent)
    : QWidget(parent)
    , m_projectAction(btn->defaultAction())
    , m_button(btn)
    , m_project(0)
    , m_target(0)
    ...
    , m_hideOnRelease(false)
{
    ...
    m_projectAction->setIcon(style()->standardIcon(QStyle::SP_ComputerIcon));
    m_projectAction->setProperty("titledAction", true);
    ...
    m_summaryLabel->setStyleSheet(QString::fromLatin1("background: %1; color: white;")
...
//        alignedWithActionHeight = m_button->height() - statusBar->height();
    int bottomMargin = 9;
    ...
    QMainWindow *mw = Core::Storage::mainWindow()->widget();
    QPoint moveTo = m_button->mapToGlobal(m_button->pos());
    QPoint pos = moveTo - mw->pos();
    bool isUpToButton = ( pos.y() > mw->height() / 2 );
    bool isLeftToButton = ( pos.x() > mw->width() / 2 );
    if ( isUpToButton )
        moveTo -= QPoint(0, height());
    else
        moveTo += QPoint(0, m_button->height());
    if ( isLeftToButton )
         moveTo -= QPoint(width() - m_button->width(), 0);

    moveTo -= QPoint(m_button->pos().x(), 0);
...
{
    int maxCount = 0;
    foreach (Project *p, ProjectManager::projects())
