...
static const char TOOLCHAIN_FILENAME[] = "/.settings/toolchains.xml";
...
{
    QString settingsLocation = Utils::FileUtils::applicationDataLocation();
    return FileName::fromString(settingsLocation + path);
}
    ...
    QObject(parent)
{
    ...
    connect(ProjectManager::instance(), SIGNAL(saveSettingsRequested()),
...
static QList<ToolChain *> restoreFromFile(const FileName &fileName)
{
    ...
    QList<ToolChainFactory *> factories = ProjectManager::toolChainFactories();
...
void ToolChainManager::restoreToolChains()
{
    ...
    QString systemSettingsFile = qApp->applicationDirPath();
    QList<ToolChain *> readTcs =
            restoreFromFile(FileName::fromString(systemSettingsFile + QLatin1String(TOOLCHAIN_FILENAME)));
...
void ToolChainManager::saveToolChains()
{
    ...
    d->m_writer->save(data, Core::Storage::mainWindow()->widget());
