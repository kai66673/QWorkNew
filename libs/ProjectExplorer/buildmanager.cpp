    ...
    : QObject(parent)
{
    ...
    connect(ProjectManager::instance(), SIGNAL(aboutToRemoveProject(ProjectExplorer::Project*)),
...
void BuildManager::extensionsInitialized()
{
    ...
    /// TODO: !! This work if have not ITaskHandler's from plugins
    d->m_taskWindow->delayedInitialization();
}
...
void BuildManager::reportError(const QString &message)
{
    d->m_outputWindow->appendText(message + "\n", ProjectExplorer::BuildStep::ErrorMessageOutput);
}
...
IOutputPane *BuildManager::compileOutputWindow()
{
    return d->m_outputWindow;
}

IOutputPane *BuildManager::taskOutputWindow()
{
    return d->m_taskWindow;
}
...
void BuildManager::updateTaskCount()
{
    ...
    Core::Storage::mainWindow()->progressManager()->setApplicationLabel(errors > 0 ? QString::number(errors) : QString());
...
void BuildManager::startBuildQueue()
{
    ...
    if (!d->m_running) {
        ...
        ProgressManager *progressManager = Core::Storage::mainWindow()->progressManager();
