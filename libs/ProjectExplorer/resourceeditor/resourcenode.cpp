...
static bool priority(const QStringList &files)
{
    if (files.isEmpty())
        return false;
    Core::MimeType mt = Core::Storage::mainWindow()->mimeDatabase()->findByFile(files.at(0));
...
static void updateNode(ResourceFile *file, ResourceTopLevelNode *node)
{
    DocumentManager::getInstance()->expectFileChanges(node->path());
    file->save();
    DocumentManager::getInstance()->unexpectFileChanges(node->path());
    node->update();
}

static bool addFilesToResource(ResourceTopLevelNode *node, const QStringList &filePaths, QStringList *notAdded,
                     const QString &prefix, const QString &lang)
{
    QString resourceFile = node->path();
    ...
    : ProjectExplorer::FolderNode(filePath)
{
    ...
    DocumentManager::getInstance()->registerProjectFileWatcher(filePath, this);
...
{
    DocumentManager::getInstance()->deregisterProjectFileWatcher(path());
}
...
{
    return addFilesToResource(this, filePaths, notAdded, QLatin1String("/"), QString());
}
...
void ResourceTopLevelNode::handleChanges()
{
    update();
}
...
{
    return addFilesToResource(m_topLevelNode, filePaths, notAdded, m_prefix, m_lang);
}
...
bool ResourceFolderNode::removeFiles(const QStringList &filePaths, QStringList *notRemoved)
{
    ...
    updateNode(&file, m_topLevelNode);
