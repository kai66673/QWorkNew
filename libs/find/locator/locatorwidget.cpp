...
LocatorWidget::LocatorWidget() :
    m_locatorManager(LocatorManager::instance()),
...
//    connect(m_refreshAction, SIGNAL(triggered()), m_locatorManager, SLOT(refresh()));
...
void LocatorWidget::startNewSearch()
{
    m_fileLineEdit->selectAll();
    setFocus();
}

bool LocatorWidget::isPopupShown() const
{
    return m_completionList->isVisible();
}
...
//    foreach (ILocatorFilter *filter, m_locatorManager->filters()) {
...
{
    QList<ILocatorFilter *> filters = m_locatorManager->filters();
...
void LocatorWidget::filterSelected()
{
    ...
    if (!currentText.isEmpty()) {
        ...
        foreach (ILocatorFilter *otherfilter, m_locatorManager->filters()) {
