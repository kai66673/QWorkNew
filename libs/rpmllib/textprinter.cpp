...
TextPrinter::TextPrinter(QObject *parent)
    : QObject(parent), parent_(0),
      printer_(new QPrinter(QPrinter::ScreenResolution)), tempdoc_(0),
      leftmargin_(15.0), rightmargin_(15.0), topmargin_(15.0),
      bottommargin_(15.0), hspacing_(3.0), fspacing_(3.0),
      headersize_(0.0), headerrule_(0.5), headerrulecolor_(QColor(Qt::black)), headertext_(QString()),
      footersize_(0.0), footerrule_(0.5), footerrulecolor_(QColor(Qt::black)), footertext_(QString()),
      dateformat_()
{
...
// spacing()
///////////////////////////////////////////////////////////////////////////////
/// Return the spacing between the page content and the header
/// blocks. This is defined in millimeters.
///////////////////////////////////////////////////////////////////////////////

double TextPrinter::hspacing() const
{
    return hspacing_;
}

///////////////////////////////////////////////////////////////////////////////
/// Return the spacing between the page content and footer
/// blocks. This is defined in millimeters.
///////////////////////////////////////////////////////////////////////////////

double TextPrinter::fspacing() const
{
    return fspacing_;
}
...
void TextPrinter::setSpacing(double spacing)
{
    if ((spacing > 0) && (spacing <= printer_->paperRect().height() / 8)) {
        hspacing_ = fspacing_ = spacing;
    } else {
        hspacing_ = fspacing_ = 0;
    }
}

void TextPrinter::setHeaderSpacing(double spacing)
{
    if ((spacing > 0) && (spacing <= printer_->paperRect().height() / 8)) {
        hspacing_ = spacing;
    } else {
...
void TextPrinter::setFooterSpacing(double spacing)
{
    if ((spacing > 0) && (spacing <= printer_->paperRect().height() / 8)) {
        fspacing_ = spacing;
    } else {
...
void TextPrinter::previewReport( RPML::ReportData *reportData,
                                 const QString &caption )
{
#if (QT_VERSION >= QT_VERSION_CHECK(4, 4, 0))
    if ( !reportData )
    ...
    const qreal delta = 5.0;
    setDateFormat("dd.MM.yyyy");
    setPageSize( reportData->m_pageFormat->printerPageSize() );
    setOrientation( reportData->m_pageFormat->isPortrait() ? QPrinter::Portrait : QPrinter::Landscape );

    Utils::PageFormat::Margin mmMargins = reportData->m_pageFormat->mmMargins();
    setLeftMargin(mmMargins.left - delta);
    setTopMargin(mmMargins.top - delta);
    setRightMargin(mmMargins.right + delta);
    setBottomMargin(mmMargins.bottom + delta);

    Utils::PageFormat::Decoration header = reportData->m_pageFormat->header();
    setHeaderText(reportData->m_headerHtml);
    setHeaderSize(header.height);
    setHeaderRule(header.ruleWidth);
    setHeaderSpacing(header.spacing);
    headerrulecolor_ = header.ruleColor;

    Utils::PageFormat::Decoration footer = reportData->m_pageFormat->footer();
    setFooterText(reportData->m_footerHtml);
    setFooterSize(footer.height);
    setFooterRule(footer.ruleWidth);
    setFooterSpacing(footer.spacing);
    footerrulecolor_ = footer.ruleColor;

    tempdoc_ = reportData->m_reportDocument->clone();
    dialog->exec();
...
QRectF TextPrinter::contentRect(QPaintDevice *device)
{
    ...
    if (headersize_ > 0) {
        ...
        rect.adjust(0, mmToInches(hspacing_) * device->logicalDpiY(), 0, 0);
    }
    // footer
    if (footersize_ > 0) {
        rect.adjust(0, 0, 0, -mmToInches(footersize_) * device->logicalDpiY());
        rect.adjust(0, 0, 0, -mmToInches(fspacing_) * device->logicalDpiY());
    }
                            ...
                            int pagenum)
{
    ...
    if (headersize_ > 0) {
        ...
        if (headerrule_ > 0.0) {
            ...
            painter->setPen(QPen(headerrulecolor_, headerrule_ * onepoint));
        ...
        header.replace("${PC}", QString::number(tempdoc_->pageCount()));
        header.replace("${PN};", QString::number(pagenum));
        if (dateformat_.isEmpty()) {
            header.replace("${D}", QDate::currentDate().toString());
        } else {
            header.replace("${D}", QDate::currentDate().toString(dateformat_));
        }
    ...
    if (footersize_ > 0) {
        ...
        if (footerrule_ > 0.0) {
            ...
            painter->setPen(QPen(footerrulecolor_, footerrule_ * onepoint));
        ...
        footer.replace("${PC}", QString::number(tempdoc_->pageCount()));
        footer.replace("${PN}", QString::number(pagenum));
        if (dateformat_.isEmpty()) {
            footer.replace("${D}", QDate::currentDate().toString());
        } else {
            footer.replace("${D}", QDate::currentDate().toString(dateformat_));
        }
