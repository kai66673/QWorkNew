...
public:
    ...
    TasksStatus( State _state = NoTasks, int _taskCount = 0 )
        : state(_state)
...
//Q_DECLARE_METATYPE(Core::TasksStatus)

namespace Internal {
class ProgressView;
}

class CORELIB_EXPORT ProgressManager : public QObject
{
    Q_OBJECT
public:
    enum ProgressFlag {
    ...
    virtual FutureProgress *addTask( const QFuture<void> &future, const QString &title,
                                     ...
                                     bool signToAppStatusBar = true,
                                     bool isSyncTask = false ) = 0;
...
signals:
    ...
    void tasksStatusChanged( const Core::TasksStatus &status );
};
