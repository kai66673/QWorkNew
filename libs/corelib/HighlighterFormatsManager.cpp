...
{
    m_instance = this;
    m_configFileName = Core::Storage::mainWindow()->applicationDataLocation() + "/highlighter-formats.xml";
...
void HighlighterFormatsManager::save()
{
    ...
    if ( !file.open(QFile::WriteOnly | QFile::Text) ) {
        Core::Storage::warning( QObject::tr("Error On Saving Highlighter Formats:\nFile \"%1\" Cannot be Opened for Writing:\n%2.")
                                .arg(m_configFileName).arg(file.errorString()) );
        return;
    }
...
void HighlighterFormatsManager::restore()
{
    QFile file(m_configFileName);
