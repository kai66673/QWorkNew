...
QString TextEditorWidget::mimeType() const
{
    return d->m_document->mimeType();
}

void TextEditorWidget::setMimeType(const QString &mt)
{
...
ITextMarkable *TextEditorWidget::markableInterface() const
{
    return baseTextDocument()->documentMarker();
}

BaseTextEditor *TextEditorWidget::editor() const
{
    if (!d->m_editor) {
        d->m_editor = const_cast<TextEditorWidget *>(this)->createEditor();
        d->m_editor->setCompletionProvider(m_completitionProvider);
        d->m_codeAssistant->configure(d->m_editor);
        connect(this, SIGNAL(textChanged()),
                d->m_editor, SIGNAL(contentsChanged()));
        connect(this, SIGNAL(changed()),
                d->m_editor, SIGNAL(changed()));
        connect(this, SIGNAL(fontSizeIncrease()), d->m_editor, SLOT(fontSizeIncrease()));
        connect(this, SIGNAL(fontSizeDecrease()), d->m_editor, SLOT(fontSizeDecrease()));
    }
    return d->m_editor;
}
...
void TextEditorWidget::duplicateSelectedLines()
{
    QTextCursor tc = textCursor();
    QTextCursor selstart = tc;
    selstart.setPosition(tc.selectionStart());
    selstart.movePosition(QTextCursor::StartOfBlock);
    QTextCursor selend = tc;
    selend.setPosition(tc.selectionEnd());
    if ( selend.atBlockStart() && selstart.blockNumber() != selend.blockNumber() )
        selend.movePosition(QTextCursor::Left, QTextCursor::KeepAnchor);
    selend.movePosition(QTextCursor::EndOfBlock);
    tc.setPosition(selstart.position());
    tc.setPosition(selend.position(), QTextCursor::KeepAnchor);

    QString duplText = tc.selectedText();
    if ( !duplText.endsWith("\n") )
        duplText.append("\n");

    if ( selend.blockNumber() == document()->lastBlock().blockNumber() ) {
        tc.setPosition(selend.selectionEnd());
        tc.insertText("\n");
    }

    tc.setPosition(selend.selectionEnd() + 1);
    tc.insertText(duplText);
    tc.setPosition(selend.selectionEnd() + 1);
    tc.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, duplText.length());
    setTextCursor(tc);
}

void TextEditorWidget::moveSelectedLinesUp()
{
    QTextCursor tc = textCursor();
    QTextCursor selStart = tc;
    selStart.setPosition(tc.selectionStart());
    ...
    QTextCursor selEnd = tc;
    selEnd.setPosition(tc.selectionEnd());
    if ( selEnd.blockNumber() == selStart.blockNumber() || !selEnd.atBlockStart() ) {
        selEnd.movePosition(QTextCursor::EndOfBlock);
        selEnd.movePosition(QTextCursor::Right);
    }
    ...
    QString moveText = tc.selectedText();

    tc.insertText("");
    tc.movePosition(QTextCursor::Up);
    int pos = tc.position();
    tc.insertText(moveText);
    tc.setPosition(pos);
    tc.movePosition(QTextCursor::Right, QTextCursor::KeepAnchor, moveText.length());
    setTextCursor(tc);
}

void TextEditorWidget::moveSelectedLinesDown()
{
    QTextCursor tc = textCursor();
    QTextCursor selStart = tc;
    selStart.setPosition(tc.selectionStart());
...
bool TextEditorWidget::createNew(const QString &contents)
{
...
void TextEditorWidget::updateCannotDecodeInfo()
{
...
const Utils::ChangeSet &TextEditorWidget::changeSet() const
{
    return d->m_changeSet;
}

void TextEditorWidget::setChangeSet(const Utils::ChangeSet &changeSet)
{
...
Core::IFile *TextEditorWidget::file()
{
    return d->m_document;
}

void TextEditorWidget::editorContentsChange(int position, int charsRemoved, int charsAdded)
{
    if (d->m_animator)
        d->m_animator->finish();
...
void TextEditorWidget::slotSelectionChanged()
{
...
void TextEditorWidget::selectBlockUp()
{
    QTextCursor cursor = textCursor();
...
void TextEditorWidget::selectBlockDown()
{
    QTextCursor tc = textCursor();
...
void TextEditorWidget::copyLineUpDown(bool up)
{
    QTextCursor cursor = textCursor();
...
void TextEditorWidget::moveLineUpDown(bool up)
{
    QTextCursor cursor = textCursor();
    QTextCursor move = cursor;
...
bool TextEditorWidget::camelCaseLeft(QTextCursor &cursor, QTextCursor::MoveMode mode)
{
    int state = 0;
    enum Input {
        Input_U,
...
bool TextEditorWidget::camelCaseRight(QTextCursor &cursor, QTextCursor::MoveMode mode)
{
    int state = 0;
    enum Input {
        Input_U,
...
bool TextEditorWidget::cursorMoveKeyEvent(QKeyEvent *e)
{
    QTextCursor cursor = textCursor();
...
void TextEditorWidget::universalHelper()
{
...
void TextEditorWidget::gotoLine(int line, int column)
{
...
void TextEditorWidget::refreshDocumentCursor()
{
    setFocus();
}

int TextEditorWidget::position(ITextEditor::PositionOperation posOp, int at) const
{
    QTextCursor tc = textCursor();
...
void TextEditorWidget::convertPosition(int pos, unsigned *line, unsigned *column) const
{
...
void TextEditorWidget::duplicateFrom(TextEditorWidget *widget)
{
    if (this == widget)
        return;
...
void TextEditorWidget::setDisplayName(const QString &title)
{
    d->m_displayName = title;
    emit changed();
}

BaseTextDocument *TextEditorWidget::baseTextDocument() const
{
    return d->m_document;
}


void TextEditorWidget::setBaseTextDocument(BaseTextDocument *doc)
{
    if (doc) {
...
void TextEditorWidget::documentAboutToBeReloaded()
{
...
void TextEditorWidget::documentReloaded()
{
...
void TextEditorWidget::setDefaultPath(const QString &defaultPath)
{
    baseTextDocument()->setDefaultPath(defaultPath);
}

void TextEditorWidget::setSuggestedFileName(const QString &suggestedFileName)
{
...
void TextEditorWidget::updateCodeFoldingVisible()
{
...
void TextEditorWidget::setIndenter(Indenter *indenter)
{
...
Indenter *TextEditorWidget::indenter() const
{
...
BaseTextEditorPrivate::~BaseTextEditorPrivate()
{
    if ( m_currentLinkInterface )
        delete m_currentLinkInterface;
}
...
QRect TextEditorWidget::foldBox()
{
...
QTextBlock TextEditorWidget::foldedBlockAt(const QPoint &pos, QRect *box) const
{
...
void BaseTextEditorPrivate::removeBlockSelection(const QString &text)
{
    ...
    for (;;) {
        ...
        int startOffset = 0;
        int startPos = ts.positionAtColumn(blockText, m_blockSelection.firstVisualColumn, &startOffset);
        int endOffset = 0;
        int endPos = ts.positionAtColumn(blockText, m_blockSelection.lastVisualColumn, &endOffset);
        ...
        if ( !emptyText ) {
            ...
            if ( startOffset > 0 )
                insertText.prepend(QString(startOffset, QLatin1Char(' ')));
    ...
    if ( emptyText )
        ...
        cursor.endEditBlock();
        q->setTextCursor(cursor);
        q->handleBlockSelection(-diff_row, 0);
        return;
    }
...
void TextEditorWidget::slotUpdateExtraAreaWidth()
{
    if (isLeftToRight())
...
void TextEditorWidget::drawFoldingMarker(QPainter *painter, const QPalette &pal,
                                       const QRect &rect,
                                       bool expanded,
                                       bool active,
                                       bool hovered) const
...
void TextEditorWidget::slotUpdateRequest(const QRect &r, int dy)
{
    if (dy)
...
void TextEditorWidget::saveCurrentCursorPositionForNavigation()
{
...
void TextEditorWidget::updateHighlights()
{
...
void TextEditorWidget::slotUpdateBlockNotify(const QTextBlock &block)
{
...
void TextEditorWidget::toggleBlockVisible(const QTextBlock &block)
{
    BaseTextDocumentLayout *documentLayout = qobject_cast<BaseTextDocumentLayout*>(document()->documentLayout());
    QTC_ASSERT(documentLayout, return);
...
const TabSettings &TextEditorWidget::tabSettings() const
{
    return d->m_document->tabSettings();
}

void TextEditorWidget::setLanguageSettingsId(const QString &settingsId)
{
    d->m_tabSettingsId = settingsId;
}

QString TextEditorWidget::languageSettingsId() const
{
...
void TextEditorWidget::onCodeStylePreferencesDestroyed()
{
...
void TextEditorWidget::handleBackspaceKey()
{
    QTextCursor cursor = textCursor();
...
void TextEditorWidget::wheelEvent(QWheelEvent *e)
{
    ...
    if (scrollWheelZoomingEnabled() && e->modifiers() & Qt::ControlModifier) {
        ...
        if (delta < 0)
            emit fontSizeIncrease();
        else if (delta > 0)
            emit fontSizeDecrease();
        return;
    }
    QPlainTextEdit::wheelEvent(e);
}

void TextEditorWidget::zoomIn(int range)
{
    d->clearVisibleFoldedBlock();
    emit requestFontZoom(range*10);
}

void TextEditorWidget::zoomOut(int range)
{
    zoomIn(-range);
}
...
void TextEditorWidget::indentInsertedText(const QTextCursor &tc)
{
    indent(tc.document(), tc, QChar::Null);
}

void TextEditorWidget::indent(QTextDocument *doc, const QTextCursor &cursor, QChar typedChar)
{
    maybeClearSomeExtraSelections(cursor);
    d->m_indenter->indent(doc, cursor, typedChar, tabSettings());
}

void TextEditorWidget::reindent(QTextDocument *doc, const QTextCursor &cursor)
{
    maybeClearSomeExtraSelections(cursor);
    d->m_indenter->reindent(doc, cursor, tabSettings());
}

Core::ILink *TextEditorWidget::findLinkAt(const QTextCursor &, bool)
{
    return 0;
}

bool TextEditorWidget::openLink( Core::ILink *link )
{
    if ( !link )
        return false;
...
void TextEditorWidget::updateLink(QMouseEvent *e)
{
    ...
    if (mouseNavigationEnabled() && e->modifiers() & Qt::ControlModifier) {
        ...
        if ( onText ) {
            if ( d->m_currentLinkInterface &&
                 d->m_currentLinkInterface->begin() <= cursor.position() &&
                 d->m_currentLinkInterface->end() >= cursor.position() )
                linkFound = true;
            else if ( Core::ILink *link = findLinkAt(cursor, false) ) {
                showLink(link);
                linkFound = true;
            }
        }
...
void TextEditorWidget::showLink( Core::ILink *link )
{
    if ( !link )
        return;

    if ( d->m_currentLinkInterface && d->m_currentLinkInterface->isEqual(link) )
        return;
    ...
    if ( d->m_currentLinkInterface )
        delete d->m_currentLinkInterface;
    d->m_currentLinkInterface = link;
    d->m_linkPressed = false;
}

void TextEditorWidget::clearLink()
{
    if ( !d->m_currentLinkInterface )
        return;
...
void TextEditorWidget::markBlocksAsChanged(QList<int> blockNumbers)
{
...
void TextEditorWidget::highlightSearchResults(const QString &txt, Find::FindFlags findFlags)
{
    QString pattern = txt;
    if (pattern.size() < 2)
...
void TextEditorWidget::setFindScope(const QTextCursor &/*start*/, const QTextCursor &/*end*/,
...
void TextEditorWidget::_q_animateUpdate(int position, QPointF lastPos, QRectF rect)
{
    QTextCursor cursor(textCursor());
    cursor.setPosition(position);
...
void TextEditorWidget::_q_matchParentheses()
{
    if (isReadOnly())
        return;
    ...
    if (backwardMatchType == TextBlockUserData::NoMatch && forwardMatchType == TextBlockUserData::NoMatch) {
        setExtraSelections(ParenthesesMatchingSelection, extraSelections_); // clear
        return;
    }
    ...
    setExtraSelections(ParenthesesMatchingSelection, extraSelections_);
}

void TextEditorWidget::_q_highlightBlocks()
{
...
void TextEditorWidget::setActionHack(QObject *hack)
{
    d->m_actionHack = hack;
}

QObject *TextEditorWidget::actionHack() const
{
...
void TextEditorWidget::maybeSelectLine()
{
    QTextCursor cursor = textCursor();
    if (!cursor.hasSelection()) {
        const QTextBlock &block = cursor.block();
...
void TextEditorWidget::maybeClearSomeExtraSelections(const QTextCursor &cursor)
{
...
void TextEditorWidget::setIfdefedOutBlocks(const QList<TextEditorWidget::BlockRange> &blocks)
{
    QTextDocument *doc = document();
    BaseTextDocumentLayout *documentLayout = qobject_cast<BaseTextDocumentLayout*>(doc->documentLayout());
    QTC_ASSERT(documentLayout, return);
...
{
    if ( QMenu *menu = DocumentManager::getInstance()->editMenu() ) {
...
void TextEditorWidget::setFontSettings(const TextEditor::FontSettings &fs)
{
...
void TextEditorWidget::setTabSettings(const TabSettings &ts)
{
...
void TextEditorWidget::setTextCodec(QTextCodec *codec)
{
    baseTextDocument()->setCodec(codec);
}

QTextCodec *TextEditorWidget::textCodec() const
{
...
BaseTextEditor::BaseTextEditor(TextEditorWidget *editor)
  : e(editor)
  , m_completitionProvider(0)
{
    setWidget(e);
    ...
    BaseTextFind *baseTextFind = new BaseTextFind(editor, this);
...
QTextDocument *BaseTextEditor::document() const
{
    return e->document();
}

void BaseTextEditor::waitForEditorSupport()
{
    e->waitForEditorSupport();
}
...
bool BaseTextEditor::isReadOnly() const
{
    return e->isReadOnly();
}

void BaseTextEditor::saveContentState( QDomDocument *document, QDomElement *element ) const
{
    Q_UNUSED(document)
    element->setAttribute( "ContentState",
                           Utils::Hex::toHexString<QByteArray>(e->saveState()));
}

void BaseTextEditor::restoreContentState( QDomElement *element )
{
    QString stateString = element->attribute("ContentState", "");
    if ( !stateString.isEmpty() )
        e->restoreState( Utils::Hex::fromHexString<QByteArray>(stateString) );
}
...
void BaseTextEditor::switchCase( ITextEditor::CaseTransform caseType )
{
    switch ( caseType ) {
        case ITextEditor::Lower:
            e->lowercaseSelection();
            break;
        case ITextEditor::Upper:
            e->uppercaseSelection();
            break;
    }
}

void BaseTextEditor::fontSizeIncrease()
{
    DocumentManager::getInstance()->fontSizeIncrease();
}

void BaseTextEditor::fontSizeDecrease()
{
...
Internal::RefactorMarkers TextEditorWidget::refactorMarkers() const
{
    return d->m_refactorOverlay->markers();
}

void TextEditorWidget::setRefactorMarkers(const Internal::RefactorMarkers &markers)
{
    foreach (const RefactorMarker &marker, d->m_refactorOverlay->markers())
...
void TextEditorWidget::handleBlockSelection(int diff_row, int diff_col)
{
...
*/
void TextEditorWidget::transformSelection(Internal::TransformationMethod method)
{
    QTextCursor cursor = textCursor();
...
void TextEditorWidget::initializeInternal( bool isInfoWidget )
{
    HighlighterFormatData *formatData = HighlighterFormatsManager::highlighterFormatData(tr("General"));
    for (int i = 0; i < FormatsCount; i++)
        formatData->fillTextCharFormat(i, m_charFormats[i]);

    const QMap<QString, QVariant> &valuesMap = SettingsManager::instance()->valuesMap("000_TextEditor");

    bool antialias = valuesMap["Antialias"].toBool();
    int zoom = valuesMap["Zoom"].toInt();

    QPalette p = palette();
    p.setColor(QPalette::Text,              m_charFormats[Text].foreground().color());
    p.setColor(QPalette::Foreground,        m_charFormats[Text].foreground().color());
    p.setColor(QPalette::Base,              m_charFormats[Text].background().color());
    p.setColor(QPalette::Highlight,         m_charFormats[Selection].background().color());
    p.setColor(QPalette::HighlightedText,   m_charFormats[Selection].foreground().color());
    p.setBrush(QPalette::Inactive, QPalette::Highlight, p.highlight());
    p.setBrush(QPalette::Inactive, QPalette::HighlightedText, p.highlightedText());
    setPalette(p);

    QFont baseFont = qvariant_cast<QFont>(valuesMap["Font"]);
    int   fontSize = baseFont.pointSize();
    fontSize = fontSize * zoom/100.;
    baseFont.setPointSize(fontSize);
    baseFont.setStyleStrategy(antialias ? QFont::PreferAntialias : QFont::NoAntialias);

    QFont currentLineFont = baseFont;
    currentLineFont.setBold(m_charFormats[CurrentLine].font().bold());
    currentLineFont.setItalic(m_charFormats[CurrentLine].font().italic());

    d->m_currentLineFormat.setFont(currentLineFont);
    d->m_currentLineFormat.setBackground(m_charFormats[CurrentLine].background());
    d->m_currentLineFormat.setForeground(m_charFormats[CurrentLine].foreground());

    QFont textFont = baseFont;
    textFont.setBold(m_charFormats[Text].font().bold());
    textFont.setItalic(m_charFormats[Text].font().italic());
    setFont(textFont);
    d->m_extraArea->setFont(textFont);


    // Line numbers
    QPalette ep = d->m_extraArea->palette();
    ep.setColor(QPalette::Dark, m_charFormats[LineNumber].foreground().color());
    ep.setColor(QPalette::Background, m_charFormats[LineNumber].background().color());
    d->m_extraArea->setPalette(ep);

    // Current Line Number
    QFont currentLineNumberFont = baseFont;
    currentLineNumberFont.setBold(m_charFormats[CurrentLineNumber].font().bold());
    currentLineNumberFont.setItalic(m_charFormats[CurrentLineNumber].font().italic());
    d->m_currentLineNumberFormat.setForeground(m_charFormats[CurrentLineNumber].foreground());
    d->m_currentLineNumberFormat.setFont(currentLineNumberFont);

    TabSettings tabSettings;
    tabSettings.m_spacesForTabs         = valuesMap["TabToSpaces"].toBool();
    tabSettings.m_autoSpacesForTabs     = false;
    tabSettings.m_autoIndent            = valuesMap["AutoIndentation"].toBool();
    tabSettings.m_smartBackspace        = valuesMap["BackspaceIndentation"].toBool();
    tabSettings.m_tabSize               = valuesMap["TabSize"].toInt();
    tabSettings.m_indentSize            = valuesMap["IndentSize"].toInt();
    tabSettings.m_indentBraces          = false;
    tabSettings.m_doubleIndentBlocks    = false;
    tabSettings.m_tabKeyBehavior        = (TabSettings::TabKeyBehavior)0;
    setTabSettings(tabSettings);

    setMouseNavigationEnabled(valuesMap["MouseNavigation"].toBool());
    setScrollWheelZoomingEnabled(valuesMap["ScrollWheelZooming"].toBool());

    /// Display Settings
    DisplaySettings ds;

    ds.m_displayLineNumbers             = valuesMap["ShowLineNumbers"].toBool();
    ds.m_textWrapping                   = valuesMap["Wrapping"].toBool();
    ds.m_showWrapColumn                 = valuesMap["ShowRightMargin"].toBool();
    ds.m_wrapColumn                     = valuesMap["RightMarginColumn"].toInt();
    ds.m_visualizeWhitespace            = valuesMap["ShowWhitespaces"].toBool();
    ds.m_displayFoldingMarkers          = valuesMap["ShowFoldingMarkers"].toBool();
    ds.m_highlightCurrentLine           = valuesMap["HighlightCurrentLine"].toBool();
    ds.m_highlightBlocks                = valuesMap["HighlightBlocks"].toBool();
    ds.m_animateMatchingParentheses     = valuesMap["AnimateMatchingParentheses"].toBool();
    ds.m_markTextChanges                = valuesMap["MarkTextChanges"].toBool();
    ds.m_autoFoldFirstComment           = false;
    ds.m_centerCursorOnScroll           = valuesMap["CenterCursorOnScroll"].toBool();
...
//    setCodeFoldingVisible(ds.m_displayFoldingMarkers);
    if ( isInfoWidget )
        ...
        setCodeFoldingSupported(ds.m_displayFoldingMarkers);
    setHighlightCurrentLine(ds.m_highlightCurrentLine);
    if ( isInfoWidget )
...
void TextEditorWidget::onSettingsChanged()
{
    initializeInternal(false);
}

void TextEditorWidget::initializeAsTextInfoWidget()
{
...
IAssistInterface *TextEditorWidget::createAssistInterface(AssistKind kind,
...
void TextEditorWidget::insertText(const QString &text)
{
    if ( d->m_inBlockSelectionMode )
        d->removeBlockSelection(text);
    else
...
void TextEditorWidget::appendText(const QString &text)
{
    if ( d->m_inBlockSelectionMode )
        d->removeBlockSelection(text);
    else
