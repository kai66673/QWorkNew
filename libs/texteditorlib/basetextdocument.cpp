...
bool BaseTextDocument::save(QString *errorString, const QString &fileName, bool autoSave, bool forceWithoutUTF8BOM )
{
    ...
    if (saveFormat.codec && saveFormat.codec->name() == "UTF-8") {
        if (forceWithoutUTF8BOM) {
...
bool BaseTextDocument::open(QString *errorString, const QString &fileName, const QString &realFileName)
{
    ...
    if (!fileName.isEmpty()) {
        ...
        if (chunks == 0) {
            ...
            Core::ProgressManager *progressManager = Core::Storage::mainWindow()->progressManager();
            interface.setProgressRange(0, chunks);
            progressManager->addTask(interface.future(), tr("Opening file"), Constants::TASK_OPEN_FILE, 0, true, true);
