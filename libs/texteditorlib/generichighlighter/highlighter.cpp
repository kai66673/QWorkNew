...
void Highlighter::onSettingsChanged()
{
    HighlighterFormatData *formatData = HighlighterFormatsManager::highlighterFormatData(tr("Generic"));
    if ( !formatData )
        return;

    for (int i = 1; i < TextFormatCount; i++)
        formatData->fillTextCharFormat(i - 1, m_formats[i]);
}
...
void Highlighter::highlightBlock(const QString &text)
{
    ...
    applyFormatToSpaces(text, m_formats[VisualWhitespace]);
}
...
//    qDebug() << "Highlighter::applyFormat" << offset << count << formatId;
    switch ( formatId ) {
        case Keyword:
        case DataType:
        case Decimal:
        ...
        case RegionMarker:
        case Others:
            setFormat(offset, count, m_formats[formatId]);
            break;
        default:
            break;
    }
