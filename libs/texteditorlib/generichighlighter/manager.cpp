...
#include <Qt4Concurrent/runextensions.h>
...
ManagerProcessor::ManagerProcessor()
    : m_knownSuffixes(QSet<QString>::fromList(Core::Storage::mainWindow()->mimeDatabase()->suffixes()))
{
...
//        m_definitionsPaths.append(settings.fallbackDefinitionFilesPath());
    ...
    m_definitionsPaths.append(QApplication::applicationDirPath() + QLatin1String("/generic-highlighter"));
...
void Manager::registerMimeTypes()
{
    if (!m_registeringWatcher.isRunning()) {
        ...
        Core::Storage::mainWindow()->progressManager()->addTask( future,
...
//        }
        foreach (const Core::MimeType &mimeType, result.second)
            Core::Storage::mainWindow()->mimeDatabase()->addMimeType(mimeType);
