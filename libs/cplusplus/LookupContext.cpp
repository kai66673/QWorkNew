...
bool CreateBindings::visit(ExecSqlStatement *sqlStatement)
{
    ClassOrNamespace *previous = enterClassOrNamespaceBinding(sqlStatement);

    // qDebug() << " >> CreateBindings::visit(ExecSqlStatement *sqlStatement)";
    for (unsigned i = 0; i < sqlStatement->memberCount(); i++)
        process(sqlStatement->memberAt(i));
