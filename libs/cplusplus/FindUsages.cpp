...
bool FindUsages::visit(ExecSqlStatementAST *ast)
{
    SimpleNameListAST *iter;

    for ( iter = ast->var_name_list; iter; iter = iter->next )
        reportResult(iter->value->identifier_token, identifier(iter->value->identifier_token));

    for ( iter = ast->table_name_list; iter; iter = iter->next )
        reportResult(iter->value->identifier_token, identifier(iter->value->identifier_token));
...
bool FindUsages::visit(QualifiedNameAST *ast)
{
    ...
    if (NameAST *unqualified_name = ast->unqualified_name) {
        ...
        if (SimpleNameAST *simple_name = unqualified_name->asSimpleName())
            ...
            identifier_token = dtor_name->unqualified_name->firstToken();
...
bool FindUsages::visit(DestructorNameAST *ast)
{
    ...
    // unsigned identifier_token = ast->identifier_token;
    reportResult(ast->unqualified_name->firstToken(), ast->name);
    return false;
}
...
bool FindUsages::visit(MemberAccessAST *ast)
{
    ...
    if (ast->member_name) {
        ...
        } else if (QualifiedNameAST *qualifiedName = ast->member_name->asQualifiedName()) {
            accept(qualifiedName);
        } else if (TemplateIdAST *tId = ast->member_name->asTemplateId()) {
            if (identifier(tId->identifier_token) == _id)
                checkExpression(ast->firstToken(), tId->identifier_token);
