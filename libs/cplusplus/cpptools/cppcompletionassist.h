...
private:
    ...
    bool isValidPlaceToProposeVariableNameOrNewType(unsigned *line, unsigned *column);
    bool checkType(const QString &typeText, CPlusPlus::Scope *scope, CPlusPlus::Document::Ptr doc);
    int proposeVariableName();
    int proposeNewOpType();
    ...
    bool sqlEmbeddedKeywordsWanted() const;
    ...
    int  completeGrammarKeywords();
    int  completeGrammarSemanticType();
    ...
    void completeNamespace(CPlusPlus::ClassOrNamespace *binding, bool checkFirstChar = false);
    ...
    void addCompletionItem(CPlusPlus::Symbol *symbol, bool checkFirstChar = false);
    void addSnippets();
    void addKeywords();
...
public:
    CppCompletionAssistInterface(QTextDocument *document,
        ...
        : TextEditor::DefaultAssistInterface(document, position, file->fileName(), reason)
        , m_snapshot(snapshot)
