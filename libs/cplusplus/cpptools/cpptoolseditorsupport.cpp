...
{
    if (_textEditor) {
        if (TextEditor::TextEditorWidget *ed = qobject_cast<TextEditor::TextEditorWidget *>(_textEditor->widget()))
...
void CppEditorSupport::updateDocument()
{
    ...
    if (qobject_cast<TextEditor::TextEditorWidget*>(_textEditor->widget()) != 0) {
