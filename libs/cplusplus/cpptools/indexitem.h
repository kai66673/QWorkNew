...
public:
                      ...
                      CPlusPlus::Symbol *symbol)
    {
        return Ptr(new IndexItem(
                       symbolName, symbolType, symbolScope, type, fileName, line, column, icon, symbol));
    }
    ...
    CPlusPlus::Symbol *symbol() { return m_symbol; }
    ...
    typedef std::function<VisitorResult (bool isSourceFile, const IndexItem::Ptr &)> Visitor;

    VisitorResult visitAllChildren(bool isSourceFile, Visitor callback) const
    {
        VisitorResult result = Recurse;
        foreach (const IndexItem::Ptr &child, m_children) {
            result = callback(isSourceFile, child);
            switch (result) {
            case Break:
                return Break;
            case Continue:
            ...
            case Recurse:
                if (!child->m_children.isEmpty()) {
                    result = child->visitAllChildren(isSourceFile, callback);
                    if (result == Break)
                        return Break;
                }
            }
