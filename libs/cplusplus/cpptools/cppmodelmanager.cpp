...
void CppPreprocessor::setHeaderPaths(const ProjectPart::HeaderPaths &headerPaths)
{
    m_headerPaths.clear();

    for (int i = 0; i < headerPaths.size(); ++i) {
        const QString &path = headerPaths.at(i).path;

#ifdef Q_OS_DARWIN
        if (i + 1 < headerPaths.size() && path.endsWith(QLatin1String(".framework/Headers"))) {
            const QFileInfo pathInfo(path);
            const QFileInfo frameworkFileInfo(pathInfo.path());
            const QString frameworkName = frameworkFileInfo.baseName();

            const QFileInfo nextIncludePath = headerPaths.at(i + 1);
...
QString CppPreprocessor::tryIncludeFile_helper(QString &fileName, IncludeType type, unsigned *revision)
{
    ...
    foreach (const QString &includePath, m_headerPaths) {
...
void CppPreprocessor::sourceNeeded(QString &fileName, IncludeType type, unsigned line)
{
    ...
    if (m_currentDoc) {
        ...
        if (contents.isEmpty() && ! QFileInfo(fileName).isAbsolute()) {
            ...
            Utils::DiagnosticMessage d(Utils::DiagnosticMessage::Warning,
    ...
    doc = Document::create(fileName, m_modelManager->precompiledHeader());
...
CppModelManager *CppModelManager::g_instance = 0;
...
//            this, SLOT(onAboutToUnloadSession()));
    ...
    connect(DocumentManager::getInstance(), SIGNAL(editorOpened(Core::IEditor*)), this, SLOT(editorOpened(Core::IEditor *)));

//    connect(m_core->editorManager(), SIGNAL(editorAboutToClose(Core::IEditor *)),
//        this, SLOT(editorAboutToClose(Core::IEditor *)));
    connect(DocumentManager::getInstance(), SIGNAL(editorClosed(Core::IEditor*)), this, SLOT(editorClosed(Core::IEditor*)));

    BuiltinIndexingSupport *support = new BuiltinIndexingSupport;
    m_headerExtensions = support->headerExtensions();
    m_sourceExtensions = support->sourceExtensions();
    m_internalIndexingSupport = support;

    connect(&m_updatedProjectsWatcher, SIGNAL(finished()), this, SLOT(updateAllDocuments()));
}

CppModelManager::~CppModelManager()
{
    m_codeModelFuture.cancel();
    delete m_findReferences;
...
void CppModelManager::ensureUpdated()
{
    ...
    m_includePaths = internaHeaderPaths();
    m_frameworkPaths = internalFrameworkPaths();
    m_definedMacros = internalDefinedMacros();
    m_precompiledHeader = internalPrecompiledHeader();
...
ProjectPart::HeaderPaths CppModelManager::internaHeaderPaths() const
{
...
ProjectPart::HeaderPaths CppModelManager::internalFrameworkPaths() const
{
    ...
    while (it.hasNext()) {
        ...
        foreach (const ProjectPart::Ptr &part, pinfo.projectParts()) {
            foreach (const ProjectPart::HeaderPath &path, part->headerPaths) {
                if ( path.type == ProjectPart::HeaderPath::FrameworkPath ) {
                    const ProjectPart::HeaderPath hp(QDir::cleanPath(path.path), path.type);
                    if (!frameworkPaths.contains(hp))
                        frameworkPaths += hp;
                }
            }
        }
...
QString CppModelManager::internalPrecompiledHeader() const
{
    QString precompiledHeader = "";
    /// TODO: !
//    for (int i = 0; i < m_projects.size(); i++)
//        precompiledHeader = m_projects[i].precompiledHeader;
...
void CppModelManager::findUsages(CPlusPlus::Symbol *symbol, const CPlusPlus::LookupContext &context)
{
    if ( symbol->identifier() ) {
        CppFindReferences *findReferences = new CppFindReferences(this);
        findReferences->findUsages(symbol, context);
    }
}
                                   ...
                                   const QString &replacement)
{
    if (symbol->identifier()) {
        CppFindReferences *findReferences = new CppFindReferences(this);
        findReferences->renameUsages(symbol, context, replacement);
    }
}
...
{
    CppFindReferences *findReferences = new CppFindReferences(this);
    findReferences->findMacroUses(macro);
}
...
QList<CppTools::ProjectInfo> CppModelManager::projectInfos() const
{
    QMutexLocker locker(&mutex);
    return m_projects.values();
}

CppTools::ProjectInfo CppModelManager::projectInfo(ProjectExplorer::Project *project) const
{
    QMutexLocker locker(&mutex);
...
QFuture<void> CppModelManager::updateProjectInfo(const ProjectInfo &pinfo)
{
    ...
    QFuture<void> result = updateSourceFiles(pinfo.sourceFiles().toList());
    m_updatedProjectsWatcher.setFuture(result);

    return result;
}

void CppModelManager::updateAllDocuments()
{
    DocumentManager::getInstance()->onFormatSettingsChanged();
}

QString CppModelManager::precompiledHeader() const
{
    QMutexLocker locker(&mutex);
    return m_precompiledHeader;
}

void CppModelManager::reinitializeExtensions()
{
    if ( BuiltinIndexingSupport *support = dynamic_cast<BuiltinIndexingSupport *>(m_internalIndexingSupport) ) {
        support->reinitializeExtensions();
        m_headerExtensions = support->headerExtensions();
        m_sourceExtensions = support->sourceExtensions();
    }
}
...
{
    QMutexLocker locker(&mutex);
    return m_fileToProjectParts.value(TO_UPPER_OPT(fileName));
}
...
void CppModelManager::forceUpdateEditor( TextEditor::ITextEditor *editor )
{
    if ( isEditorSupported(editor) ) {
        CppEditorSupport *editorSupport = m_editorSupport.value(editor);
        editorSupport->updateDocumentNow();
    }
}
...
void CppModelManager::editorClosed(Core::IEditor *editor)
{
    if (isEditorSupported(editor)) {
        TextEditor::ITextEditor *textEditor = qobject_cast<TextEditor::ITextEditor *>(editor);
        QTC_ASSERT(textEditor, return);
...
bool CppModelManager::isEditorSupported(Core::IEditor *editor) const
{
    return editor->context().contains(CppEditor::Constants::C_CPPEDITOR);
}
...
void CppModelManager::updateEditor(Document::Ptr doc)
{
    ...
    Core::IEditor *editor = DocumentManager::getInstance()->editorForFileName(fileName);
...
//        if (editor->file()->fileName() == fileName) {
            ...
            TextEditor::TextEditorWidget *ed = qobject_cast<TextEditor::TextEditorWidget *>(textEditor->widget());
            if (! ed)
                return;

            QList<TextEditor::TextEditorWidget::BlockRange> blockRanges;

            foreach (const Document::Block &block, doc->skippedBlocks()) {
                blockRanges.append(TextEditor::TextEditorWidget::BlockRange(block.begin(), block.end()));
            }
...
#else
            QSet<int> lines;
            QList<Utils::DiagnosticMessage> messages = doc->diagnosticMessages();
            messages += extraDiagnostics(doc->fileName());
            foreach (const Utils::DiagnosticMessage &m, messages) {
                if (m.fileName() != fileName)
                    continue;
...
void CppModelManager::updateEditorSelections()
{
    foreach (const Editor &ed, m_todo) {
        ...
        TextEditor::TextEditorWidget *editor = qobject_cast<TextEditor::TextEditorWidget *>(textEditor->widget());
        ...
        editor->setExtraSelections(TextEditor::TextEditorWidget::CodeWarningsSelection,
                                   ed.selections);
...
QList<Utils::DiagnosticMessage> CppModelManager::extraDiagnostics(const QString &fileName, int kind) const
{
    QMutexLocker locker(&protectExtraDiagnostics);
    if (kind == -1) {
        QList<Utils::DiagnosticMessage> messages;
        foreach (const QList<Utils::DiagnosticMessage> &list, m_extraDiagnostics.value(fileName))
            messages += list;
        return messages;
    }
...
const CppHelperItem *CppModelManager::cppHelper(const QString &key, ITextDocument *textDocument, const QString &filePath)
{
    CppHelperDialog helperDialog(filePath, key, Core::Storage::mainWindow()->widget());
    if ( helperDialog.exec() == QDialog::Accepted ) {
        const CppTools::CppHelperItem *item = helperDialog.selectedItem();
        if ( item )
            textDocument->replaceCurrentWord(item->m_qualifiedName);
        return item;
    }
...
QString CppModelManager::bestIncludeCandidate( const QString &fileName, const QString &fullIncludePath )
{
    QStringList localDirs;
    ProjectPart::HeaderPaths globalDirs;
    localDirs << QFileInfo(fileName).path();
    globalDirs = allHeaderPaths();

    QStringList candidates;
    foreach ( const QString &localDirName, localDirs) {
        QDir dir(localDirName);
        candidates << ("\"" + dir.relativeFilePath(fullIncludePath) + "\"");
    }
    foreach ( const ProjectPart::HeaderPath &globalDirName, globalDirs ) {
        QDir dir(globalDirName.path);
        candidates << ("<" + dir.relativeFilePath(fullIncludePath) + ">");
    }
    candidates << ("\"" + fullIncludePath + "\"");

    int minimalLength = candidates[0].length();
    int minimalIndex = 0;

    for (int i = 1; i < candidates.size(); i++)
        if ( candidates[i].length() < minimalLength ) {
            minimalIndex = i;
            minimalLength = candidates[i].length();
        }

    return candidates[minimalIndex];
}
    ...
    foreach (const ProjectInfo &projectInfo, m_projects) {
        foreach (const ProjectPart::Ptr &projectPart, projectInfo.projectParts()) {
            ...
            foreach (const ProjectFile &cxxFile, projectPart->files)
                m_fileToProjectParts[TO_UPPER_OPT(cxxFile.path)].append(projectPart);

        }
    }
}

void CppModelManager::finishCppHelper(const CppHelperItem *item, Document::Ptr doc, QTextDocument *textDocument)
{
    ...
    int line = 0;
    QString insertText;
    if ( !doc->includes().isEmpty() )
        line = doc->includes().last().line();
    else {  // Skip first comment lines
        line = CPlusPlus::SimpleLexer::skipFirstComment(textDocument);
    }
    if ( item->m_functionReturnType.isEmpty() ) {
        insertText = QString("#include %1\n").arg(bestIncludeCandidate(doc->fileName(), item->m_fileName));
    } else {
        insertText = QString("extern %1%2%3;\n")
...
{
    return m_internalIndexingSupport;
}

void CppModelManager::projectOpened( QVariantList &/*valuesList*/ )
{
//    ProjectInfo pinfo;
//    pinfo.localIncludePaths = valuesList[1].toStringList();
//    pinfo.globalIncludePaths = valuesList[2].toStringList();
//    QStringList defines = valuesList[3].toStringList();
//    QString strDefines = "";
//    for (int i = 0; i < defines.size(); i++) {
//        strDefines.append(QString("#define %1\n").arg(defines[i]));
//    }
//    pinfo.defines = strDefines.toLocal8Bit();
//    pinfo.sourceFiles = valuesList[4].toStringList();
//    pinfo.precompiledHeader = valuesList[5].toString();
//    pinfo.sourceFiles.append(QLatin1String("<configuration>"));
//    m_codeModelFuture.cancel();
//    updateProjectInfo(pinfo);
//    m_codeModelFuture = updateSourceFiles(pinfo.sourceFiles);
}

void CppModelManager::projectClosed( QVariantList &/*valuesList*/ )
{
    m_codeModelFuture.cancel();
...
void CppModelManager::projectChanged( QVariantList &valuesList )
{
    if ( valuesList.size() == 0 )
        return;

    int changeType = valuesList[0].toInt();
    switch ( changeType ) {
        case 0: // Settings Changed
            break;
        case 1: // Files Added
            onAddFilesToProject(valuesList[1].toStringList());
            break;
        case 2: // Files Removed
            onRemoveFilesFromProject(valuesList[1].toStringList());
            break;
        default:
            qDebug() << " !! CppModelManager::projectChanged !! Unknown Cnange Project Type";
    }
}

void CppModelManager::onAddFilesToProject( QStringList /*files*/ )
{
    /// TODO: ? AsofProject
//    if ( m_codeModelFuture.isRunning() ) {
//        QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
//        m_codeModelFuture.waitForFinished();
...
//            return;
//        m_projects[0].sourceFiles.append(files);
//        m_dirty = true;
//    } while (0);

//    m_codeModelFuture = updateSourceFiles(files);
}

void CppModelManager::onRemoveFilesFromProject( QStringList /*files*/ )
{
    /// TODO: ? AsofProject
//    if ( m_codeModelFuture.isRunning() ) {
//        QApplication::setOverrideCursor(QCursor(Qt::WaitCursor));
...
//            m_projects[0].sourceFiles.removeOne(file);
