...
ProjectFile::Kind ProjectFile::classify(const QString &file)
{
    ...
    const Core::MimeType mimeType = Core::Storage::mainWindow()->mimeDatabase()->findByFile(fi);
    if (!mimeType)
        return Unclassified;
...
{
    Core::MimeType mimeType = Core::Storage::mainWindow()->mimeDatabase()->findByType(QLatin1String(mimeName));
    if (!mimeType.isNull())
