...
{
    Utils::DiagnosticMessage m(Utils::DiagnosticMessage::Warning, _fileName, line, column, text, length);
...
//                return rangeBasedForStmt->symbol;
        ...
        } else if (ExecSqlStatementAST *sqlStatement = ast->asExecSqlStatement()) {
            if ( sqlStatement->symbol )
                return sqlStatement->symbol;
...
bool CheckSymbols::visit(NamespaceAST *ast)
{
    if (ast->identifier_token) {
        ...
        if (! tok.generated()) {
            ...
            Result use(line, column, tok.length());
            addUse(use);
        }
    }
...
{
    addUse(ast->identifier_token, Result::Static);
    return true;
}
            ...
            if (NameAST *nameAST = declaratorId(ast->declarator_list->value)) {
                if (Function *funTy = decl->type()->asFunctionType()) {
                    if (funTy->isVirtual()
                        ...
                        addUse(nameAST, Result::VirtualMethod);
...
//                        if ((_usages.back().kind != Result::VirtualMethod)) {
//                            if (funTy->isOverride())
    ...
    if (const Name *name = ast->member_name->name) {
        if (const Identifier *ident = name->identifier()) {
            ...
            if (_potentialFields.contains(id)) {
                ...
                const QByteArray expression = _doc->source()
                        .mid(start.begin(), end.end() - start.begin());
...
bool CheckSymbols::visit(NewExpressionAST *ast)
{
    ...
    if (/*highlightCtorDtorAsType*/ true) {
...
bool CheckSymbols::visit( ExecSqlStatementAST *ast )
{
    SimpleNameListAST *iter;
    SimpleNameListAST *iter1;

    for (iter = ast->field_name_list; iter; iter = iter->next ) {
        const Token &tok = tokenAt(iter->value->identifier_token);
        if ( !tok.generated() ) {
            unsigned line, column;
            getTokenStartPosition(iter->value->identifier_token, &line, &column);
            Result use(line, column, tok.length());
            use.kind = Result::Field;
            addUse(use);
        }
    }
    ...
    for ( iter = ast->table_name_list, iter1 = ast->alias_name_list;
        ...
        if ( !tok.generated() ) {
            ...
            getTokenStartPosition(iter->value->identifier_token, &line, &column);
            Result use(line, column, tok.length());
            use.kind = Result::Type;
            addUse(use);
        }
        if ( iter->value->identifier_token != iter1->value->identifier_token ) {
            const Token &tok1 = tokenAt(iter1->value->identifier_token);
            if ( !tok1.generated() ) {
                unsigned line, column;
                getTokenStartPosition(iter1->value->identifier_token, &line, &column);
                Result use(line, column, tok1.length());
                use.kind = Result::SqlTableAlias;
                addUse(use);
            }
        }
    }
        ...
        if (ast->asDestructorName() != 0) {
            ...
            if (klass) {
                if (hasVirtualDestructor(_context.lookupType(klass))) {
                    addUse(ast, Result::VirtualMethod);
                } else {
                    bool added = false;
        ...
        if (binding && ast->unqualified_name) {
            if (ast->unqualified_name->asDestructorName() != 0) {
                if (hasVirtualDestructor(binding)) {
                    addUse(ast->unqualified_name, Result::VirtualMethod);
                } else {
                    bool added = false;
                    if (/*highlightCtorDtorAsType &&*/ maybeType(ast->name))
                    ...
                    if (!added)
                        addUse(ast->unqualified_name, Result::Function);
                }
            } else {
                    ...
                    if (NameAST *class_or_namespace_name = nested_name_specifier->class_or_namespace_name) {
                        if (TemplateIdAST *template_id = class_or_namespace_name->asTemplateId()) {
                            if (template_id->template_token) {
                                addUse(template_id, Result::Type);
...
{
    if (ast->identifier_token)
        addUse(ast->identifier_token, Result::Label);
...
bool CheckSymbols::visit(LabeledStatementAST *ast)
{
    if (ast->label_token && !tokenAt(ast->label_token).isKeyword())
        addUse(ast->label_token, Result::Label);
...
bool CheckSymbols::visit(FunctionDefinitionAST *ast)
{
    ...
    if (ast->declarator && ast->symbol && ! ast->symbol->isGenerated()) {
        ...
        if (NameAST *declId = declaratorId(ast->declarator)) {
            ...
            if (fun->isVirtual()
                ...
                addUse(declId, Result::VirtualMethod);
    ...
    foreach (const QList<Result> &uses, locals.uses) {
        foreach (const Result &u, uses)
            if ( u.kind != Result::SqlTableAliasShadowed)
                addUse(u);
    }
...
void CheckSymbols::addUse(NameAST *ast, Result::Kind kind)
{
    if (! ast)
        return;
...
void CheckSymbols::addUse(unsigned tokenIndex, Result::Kind kind)
{
    if (! tokenIndex)
        return;
...
void CheckSymbols::addType(ClassOrNamespace *b, NameAST *ast)
{
    ...
    const unsigned length = tok.length();
    const Result use(line, column, length, Result::Type);
    addUse(use);
}
...
bool CheckSymbols::maybeAddField(const QList<LookupItem> &candidates, NameAST *ast)
{
    ...
    foreach (const LookupItem &r, candidates) {
        ...
        const Result use(line, column, length, Result::Field);
        addUse(use);
    ...
    foreach (const LookupItem &r, candidates) {
        ...
        if (argumentCount < funTy->minimumArgumentCount()) {
            if (matchType != Match_Ok) {
                kind = funTy->isVirtual() ? Result::VirtualMethod : Result::Function;
