                           ...
                           bool wantFunctionCall)
{
    ...
    switch (ch.toLatin1()) {
    case ' ':
        if ( ch2.isLetterOrNumber() || ch2 == QLatin1Char('>') || ch2 == QLatin1Char('_') ||
             ch2 == QLatin1Char('*') || ch2 == QLatin1Char('&') ||
             (ch2 == QLatin1Char(':') && ch3 != QLatin1Char(':')) ) {
            completionKind = T_SEMICOLON;
            referencePosition = 1;
        } else if ( ch2 == QLatin1Char('=') &&
                    ch3 != QLatin1Char('=') && ch3 != QLatin1Char('!') &&
                    ch3 != QLatin1Char('+') && ch3 != QLatin1Char('-') &&
                    ch3 != QLatin1Char('*') && ch3 != QLatin1Char('/') && ch3 != QLatin1Char('%') &&
                    ch3 != QLatin1Char('<') && ch3 != QLatin1Char('>') ) {
    ...
    case '&':
        if ( ch2.isLetterOrNumber() || ch2 == QLatin1Char('>') || ch2 == QLatin1Char('_') ||
             ch2 == QLatin1Char('*') || ch2 == QLatin1Char(' ') || ch2 == QLatin1Char('\t') ) {
            completionKind = T_SEMICOLON;
            referencePosition = 1;
        }
        break;
    ...
    case ':':
        if (ch3 != QLatin1Char(':') && ch2 == QLatin1Char(':')) {
            ...
            completionKind = T_EXEC_SQL_INCLUDE_FILENAME;
            referencePosition = 1;
        }
        break;
    case '>':
    ...
    case '*':
        if (ch2 == QLatin1Char('.')) {
            ...
            referencePosition = 3;
        } else if ( ch2.isLetterOrNumber() || ch2 == QLatin1Char('>') || ch2 == QLatin1Char('_') ||
                    ch2 == QLatin1Char('*') || ch2 == QLatin1Char(' ') || ch2 == QLatin1Char('\t') ) {
                   completionKind = T_SEMICOLON;
                   referencePosition = 1;
        }
    ...
    case '#':
        ...
        break;
    case '%':
        completionKind = T_PERCENT;
        referencePosition = 1;
        break;
    case '=':
        if ( ch2 != QLatin1Char('=') && ch2 != QLatin1Char('!') &&
             ch2 != QLatin1Char('+') && ch2 != QLatin1Char('-') &&
             ch2 != QLatin1Char('*') && ch2 != QLatin1Char('/') && ch2 != QLatin1Char('%') &&
             ch2 != QLatin1Char('<') && ch2 != QLatin1Char('>') ) {
            completionKind = T_EQUAL;
                                                    ...
                                                    int basePosition) const
{
    ...
    } else if ( m_completionOperator == T_LESS_LESS ) {
        toInsert = text();
        extraChars += QLatin1Char('>');
    } else {
        toInsert = text();
...
//                TextEditorSettings::instance()->completionSettings();
        const bool autoInsertBrackets = /*completionSettings.m_autoInsertBrackets*/ true;
        ...
        if (autoInsertBrackets && symbol && symbol->type()) {
            if (Function *function = symbol->type()->asFunctionType()) {
                ...
                if (! function->hasReturnType() && (function->unqualifiedName() && !function->unqualifiedName()->isDestructorNameId())) {
                    ...
                    if (/*completionSettings.m_spaceAfterFunctionName*/false)
                        extraChars += QLatin1Char(' ');
                    extraChars += QLatin1Char('(');
    ...
    const QString inEditor = Convenience::textAt( QTextCursor(editor->document()),
                                                  editor->position(),
    ...
    int length = editor->position() - basePosition + preserveLength + extraLength;
...
// ---------------------------
bool CppCompletionAssistProvider::isAsynchronous() const
{
    return true;
}

bool CppCompletionAssistProvider::supportsEditor( const Core::Context &context ) const
{
    return context.contains(CppEditor::Constants::C_CPPEDITOR);
}
...
// ---------------------
class CppProposalWidget: public TextEditor::GenericProposalWidget
{
protected:
    bool eventFilter(QObject *o, QEvent *e);
};

bool CppProposalWidget::eventFilter(QObject *o, QEvent *e)
{
    if ( e->type() == QEvent::ShortcutOverride ) {
        QKeyEvent *ke = static_cast<QKeyEvent *>(e);
        CppModelManagerInterface *modelManager = CppModelManagerInterface::instance();
        QKeySequence keShortcut(ke->key() | ke->modifiers());
        foreach (QAction *action, modelManager->advancedCompletitionActions()) {
            if ( keShortcut == action->shortcut() ) {
                abort();
                e->ignore();
                return true;
            }
    ...
    return TextEditor::GenericProposalWidget::eventFilter(o, e);
}
...
IAssistProposalWidget *CppAssistProposal::createWidget() const
{
    return new CppProposalWidget();
}
...
IAssistProposal * CppCompletionAssistProcessor::perform(const IAssistInterface *_interface)
{
    m_interface.reset(static_cast<const CppCompletionAssistInterface *>(_interface));

    if (_interface->reason() != ExplicitlyInvoked && !accepts())
        return 0;

    int index = -1;
    switch ( m_model->m_completionOperator ) {
        case T_SEMICOLON:   index = proposeVariableName();  break;
        case T_EQUAL:       index = proposeNewOpType();       break;
        default:            index = startCompletionHelper();
    }
...
bool CppCompletionAssistProcessor::accepts() const
{
    ...
    if (start != pos) {
        if ( token == T_POUND || token == T_PERCENT ) {
...
bool CppCompletionAssistProcessor::isValidPlaceToProposeVariableNameOrNewType(unsigned *line, unsigned *column)
{
    QTextDocument *textDocument = m_interface->textDocument();
    int pos = m_interface->position();
    QTextBlock block = textDocument->findBlock(pos);
    if ( !block.isValid() )
        return false;
    QString blockStr = block.text();
    Convenience::convertPosition(textDocument, pos, line, column);
    bool isNotValidPlace = true;
    while (isNotValidPlace && *column < ((unsigned)blockStr.length())) {
        char ch = blockStr[*column].toLatin1();
        switch ( ch ) {
            case ';':
            case '\n':
                isNotValidPlace = false;
                break;
            case ' ':
            case '\t':
                break;
...
bool CppCompletionAssistProcessor::checkType(const QString &typeText, Scope *scope, CPlusPlus::Document::Ptr doc)
{
    bool isIdentifier = true;

    if ( !typeText[0].isLetter() && typeText[0] != '_' ) {
        isIdentifier = false;
    } else {
        for (int i = 1; i < typeText.length(); i++) {
            if ( !typeText[i].isLetterOrNumber() && typeText[i] != '_'  ) {
                isIdentifier = false;
                break;
            }
        }
    ...
    if ( !isIdentifier )
        return true;    /// TODO: validate complex type expression


    m_model->m_typeOfExpression->init(doc, m_interface->snapshot());
    const QList<LookupItem> results = (*m_model->m_typeOfExpression)(typeText, scope, TypeOfExpression::Preprocess);
    foreach ( const LookupItem &item, results ) {
        if ( Symbol *d = item.declaration() ) {
            if ( d->isClass() || d->isTypedef() )
                return true;
        }
    }
...
int CppCompletionAssistProcessor::proposeVariableName()
{
    unsigned line = 0, column = 0;
    if ( !isValidPlaceToProposeVariableNameOrNewType(&line, &column) )
        return -1;

    Document::Ptr thisDocument = m_interface->snapshot().document(m_interface->fileName());
    if (! thisDocument)
        return -1;
    Scope *scope = thisDocument->scopeAt(line, column, m_interface->textDocument());
    if ( !scope )
        return -1;

    bool inClass = scope->isClass();
    ...
    int endTypePos = pos;
    while (pos >= 0) {
        char ch = m_interface->characterAt(pos).toLatin1();
        if ( ch == ';' || ch == '}' || ch == '{' || ch == '(' || ch == ')'
             || (ch == ':' && pos > 0 && m_interface->characterAt(pos - 1).toLatin1() != ':') )
            break;
        pos--;
    }
    pos++;

    QString typeText = m_interface->textAt(pos, endTypePos - pos).trimmed();

    ProposeNameResolver nameResolver(typeText);
    bool isTypedef;
    QStringList candidates = nameResolver(inClass, &isTypedef);
    if ( candidates.isEmpty() )
        return -1;

    if ( !checkType(typeText, scope, thisDocument) )
        return -1;

    Icons::IconType iconType = isTypedef ? Icons::ClassIconType : Icons::VarPublicIconType;
    for (int i = 0; i < candidates.size(); i++)
        addCompletionItem(candidates[i], m_icons.iconForType(iconType));

    m_startPosition = endTypePos + 1;
    return m_startPosition;
}

int CppCompletionAssistProcessor::proposeNewOpType()
{
    unsigned line = 0, column = 0;
    if ( !isValidPlaceToProposeVariableNameOrNewType(&line, &column) )
        return -1;

    int startPosition = m_interface->position();
    int pos = startPosition - 1;
    while ( m_interface->characterAt(pos).isSpace() || m_interface->characterAt(pos) == QLatin1Char('=') )
        pos--;
    pos++;
    ...
    QString expression = expressionUnderCursor(tc).trimmed();
    ...
    QList<LookupItem> results = (*m_model->m_typeOfExpression)(expression, scope, TypeOfExpression::Preprocess);

    if ( results.isEmpty() )
        return -1;

    foreach (const LookupItem &item, results) {
        if ( item.type()->isPointerType() ) {
            Overview overview;
            QString prettyType = overview(item.type());
            prettyType.chop(1);
            prettyType = prettyType.trimmed();
            addCompletionItem("new " + prettyType);
    ...
    if (start != pos) {
        ...
        // Don't complete in comments or strings, but still check for include completion
                 ...
                 (tk.isLiteral() && (*kind != T_STRING_LITERAL
                                     ...
                                     && *kind != T_SLASH
                                     && *kind != T_EXEC_SQL_INCLUDE_FILENAME))) {
            *kind = T_EOF_SYMBOL;
            start = pos;
        }
        ...
        // Check for include preprocessor directive
            ...
            if (tokens.size() >= 3) {
                ...
                else if ( tokens.at(0).is(T_PERCENT) && tokens.at(1).is(T_IDENTIFIER) && tokens.at(2).is(T_LESS) ) {
                    const Token &directiveToken = tokens.at(1);
                    QString directive = tc.block().text().mid(directiveToken.begin(),
                                                              directiveToken.length());
                    if ( directive == QLatin1String("token") || directive == QLatin1String("type") ) {
                        *kind = T_LESS_LESS;
                        include = true;
                    }
                }
        ...
        // Check C$INCLUDE
        else if ( *kind == T_EXEC_SQL_INCLUDE_FILENAME ) {
            bool cinclude = false;
            if ( tokenIdx > 0 ) {
                const Token &previousToken = tokens.at(tokenIdx - 1); // look at the token at the left of T_COLON
                if ( previousToken.is(T_IDENTIFIER) || previousToken.is(T_SQL_CINCLUDE) ) {
                    QString directive = tc.block().text().mid(previousToken.begin(),
                                                          previousToken.length());
                    if ( !directive.compare(QLatin1String("c$include"), Qt::CaseInsensitive) ) {
                        cinclude = true;
                }
              }
            }
            ...
            m_model->m_completionOperator = T_SEMICOLON;
            return -1;
        } else if ( *kind == T_EQUAL ) {
            m_model->m_completionOperator = T_EQUAL;
            return -1;
        }
    }
...
int CppCompletionAssistProcessor::startCompletionHelper()
{
    ...
    m_firstChar = m_interface->characterAt(startOfName);
    if ( m_firstChar.isSpace() )
        m_firstChar = ' ';
    ...
    QString fileName = m_interface->fileName();
    ...
    // Grammar Semantic Type Completition
    if ( m_model->m_completionOperator == T_LESS_LESS ) {
        m_startPosition = startOfName;
        m_startPosition = completeGrammarSemanticType();
        return m_startPosition;
    }

    if ( m_model->m_completionOperator == T_PERCENT ) {
        m_startPosition = startOfName;
        m_startPosition = completeGrammarKeywords();
        return m_startPosition;
    }
    ...
    if (m_model->m_completionOperator == T_STRING_LITERAL
        ...
        || m_model->m_completionOperator == T_EXEC_SQL_INCLUDE_FILENAME) {
...
void CppCompletionAssistProcessor::addCompletionItem(CPlusPlus::Symbol *symbol, bool checkFirstChar)
{
    ConvertToCompletionItem toCompletionItem;
    BasicProposalItem *item = toCompletionItem(symbol);
    if (item) {
        if ( checkFirstChar && !item->text().startsWith(m_firstChar, Qt::CaseInsensitive) ) {
            delete item;
            return;
        }
...
bool CppCompletionAssistProcessor::completeInclude(const QTextCursor &cursor)
{
    ...
    const Core::MimeType mimeType = Core::Storage::mainWindow()->mimeDatabase()->findByType(QLatin1String("text/x-c++hdr"));
...
int CppCompletionAssistProcessor::completeGrammarKeywords()
{
    unsigned line = 0, column = 0;
    Convenience::convertPosition(m_interface->textDocument(), m_startPosition, &line, &column);
    ...
    if ( scope->asGrammarScope() ) {
        addCompletionItem("left");
        addCompletionItem("nonassoc");
        addCompletionItem("right");
        addCompletionItem("token");
        addCompletionItem("type");
        addCompletionItem("union");
        addCompletionItem("skeleton");
        addCompletionItem("require");
        addCompletionItem("debug");
        addCompletionItem("defines");
        addCompletionItem("define api.namespace");
        addCompletionItem("define parser_class_name");
        addCompletionItem("code");
        addCompletionItem("code requires");
        addCompletionItem("parse-param");
        return m_startPosition;
    }
...
int CppCompletionAssistProcessor::completeGrammarSemanticType()
{
    unsigned line = 0, column = 0;
    Convenience::convertPosition(m_interface->textDocument(), m_startPosition, &line, &column);
    ...
    if ( GrammarScope *gs = scope->asGrammarScope() ) {
        if ( Class *unionClass = gs->unionCLass() ) {
            for ( unsigned i = 0; i < unionClass->memberCount(); i++ )
                addCompletionItem(unionClass->memberAt(i));
        }
        return m_startPosition;
    }
...
bool CppCompletionAssistProcessor::objcKeywordsWanted() const
{
    ...
    const Core::MimeDatabase *mdb = Core::Storage::mainWindow()->mimeDatabase();
    return mdb->findByFile(fileName).type() == CppTools::Constants::OBJECTIVE_CPP_SOURCE_MIMETYPE;
}

bool CppCompletionAssistProcessor::sqlEmbeddedKeywordsWanted() const
{
    if ( !m_sqlEmbeddedEnabled )
        return false;

    QString fileName = m_interface->fileName();

    const Core::MimeDatabase *mdb = Core::Storage::mainWindow()->mimeDatabase();
    return mdb->findByFile(fileName).type() == CppTools::Constants::SC_SOURCE_MIMETYPE;
}
...
{
    if ( GrammarScope *gs = currentScope->asGrammarScope() ) {
        for (unsigned i = 0; i < gs->memberCount(); ++i) {
            if ( GrammarSymbol *symbol = gs->memberAt(i)->asGrammarSymbol() ) {
                if ( symbol->usageType() == GrammarSymbol::RuleDefinitionType ||
                     symbol->usageType() == GrammarSymbol::TokenDeclarationType )
    ...
    for (; currentBinding; currentBinding = currentBinding->parent()) {
        ...
        if (! symbols.isEmpty()) {
            if (symbols.first()->isNamespace())
                completeNamespace(currentBinding, true);
            else
...
void CppCompletionAssistProcessor::completeNamespace(CPlusPlus::ClassOrNamespace *b, bool checkFirstChar)
{
    ...
    while (! bindingsToVisit.isEmpty()) {
        ...
        while (! scopesToVisit.isEmpty()) {
            ...
            for (Scope::iterator it = scope->firstMember(); it != scope->lastMember(); ++it) {
                Symbol *member = *it;
                addCompletionItem(member, checkFirstChar);
            }
        }
    }
}
...
void CppCompletionAssistProcessor::addKeywords()
{
    ...
    // SQL-Embedded
    if ( sqlEmbeddedKeywordsWanted() ) {
        for (int i = T_FIRST_SQL_KEYWORD; i <= T_LAST_SQL_KEYWORD; i++)
            addCompletionItem(QLatin1String(Token::name(i)), m_icons.sqlKeywordIcon());
    }
}
                                                                 ...
                                                                 bool toolTipOnly)
{
    ...
    if (! functions.isEmpty() && ! toolTipOnly) {
        ...
        unsigned lineSigned = 0, columnSigned = 0;
