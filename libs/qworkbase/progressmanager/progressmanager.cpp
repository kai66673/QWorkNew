...
void ProgressManagerPrivate::cancelTasks(const QString &type)
{
    ...
    if (found) {
        ...
        emit tasksStatusChanged(TasksStatus());
    }
}
...
FutureProgress *ProgressManagerPrivate::createTask( const QFuture<void> &future, const QString &type, ProgressFlags flags )
{
                                                 ...
                                                 bool signToAppStatusBar , bool isSyncTask )
{
    ...
    if ( isSyncTask )
        m_syncTask = watcher;
    TasksStatus status;
    status.taskCount = m_runningTasks.size();
    if ( m_syncTask )
        status.state = TasksStatus::SyncTaskExecuted;
    else
        status.state = TasksStatus::AnyTaskExecuted;
    emit tasksStatusChanged(status);

    if ( signToAppStatusBar )
        return m_progressView->addTask(future, title, type, flags);

    m_progressView->addTask(future, title, type, flags);
    return createTask(future, type, flags);
}
...
void ProgressManagerPrivate::taskFinished()
{
    ...
    TasksStatus status;
    int tasksCount = m_runningTasks.size();
    status.taskCount = tasksCount;
    if ( tasksCount ) {
        if ( m_syncTask )
            status.state = TasksStatus::SyncTaskExecuted;
        else
