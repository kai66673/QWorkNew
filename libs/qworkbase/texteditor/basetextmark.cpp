...
void BaseTextMark::init()
{
    ...
    connect(documentManager, SIGNAL(editorOpened(Core::IEditor*)), this, SLOT(editorOpened(Core::IEditor*)));
//    connect(documentManager, SIGNAL(editorClosed(Core::IEditor*)), this, SLOT(editorClosed(Core::IEditor*)));

    foreach (Core::IEditor *editor, documentManager->openedEditors())
        editorOpened(editor);
}
...
void BaseTextMark::moveMark(const QString & /* filename */, int /* line */)
{
    DocumentManager *documentManager = DocumentManager::getInstance();
    if (!m_init) {
        connect(documentManager, SIGNAL(editorOpened(Core::IEditor *)),
