...
QString FileUtils::applicationDataLocation()
{
...
QString FileUtils::applicationName()
{
    return qApp->applicationName();
}

bool FileUtils::renameFile(const QString &from, const QString &to)
{
...
void FileUtils::showInGraphicalShell(QWidget *parent, const QString &pathIn, QSettings *settings)
{
    ...
    if (HostOsInfo::isWindowsHost()) {
        ...
        const QString app = UnixUtils::fileBrowser(settings);
...
void FileUtils::openTerminal(const QString &path, QSettings *settings)
{
    ...
    if (hostOs == OsTypeWindows) {
        ...
        terminalEmulator = QApplication::applicationDirPath() + "/Resources"
            + QLatin1String("/scripts/openTerminal.command");
    } else {
        args = QtcProcess::splitArgs(ConsoleProcess::terminalEmulator(settings, hostOs));
...
void FileUtils::removeFile(const QString &filePath, bool deleteFromFS, QWidget *dialogParent)
{
    if (deleteFromFS) {
        QFile file(filePath);
