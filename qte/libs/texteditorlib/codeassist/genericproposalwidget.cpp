...
bool isPerfectMatch(const QString &prefix, const IGenericProposalModel *model)
{
    ...
    for (int i = 0; i < model->size(); ++i) {
        ...
        if (!current.isEmpty()) {
            TextEditor::CaseSensitivity cs = TextEditor::CaseInsensitive;
    ...
    , m_autoWidth(true)
{
    ...
    m_completionListView->setFont(qvariant_cast<QFont>(valuesMap["Font"]));
...
bool GenericProposalWidget::updateAndCheck(const QString &prefix)
{
    ...
    if (/*TextEditorSettings::completionSettings().m_partiallyComplete*/ true   ///! TODO:
            && d->m_kind == Completion
            && d->m_justInvoked
            && d->m_isSynchronized) {
        if (d->m_model->size() == 1) {
