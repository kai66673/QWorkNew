...
AbstractEditorSupport::AbstractEditorSupport(ICodeModel *codemodel) :
    m_codemodel(codemodel), m_revision(1)
{
}
...
{
    ++m_revision;
    m_codemodel->updateSourceFiles(QStringList() << fileName());
}
...
//    return Internal::CppFileSettings::licenseTemplate(file, className);
    return tr("/** Must be Implemented file = %1 class = %2*/\n").arg(file).arg(className);
}
