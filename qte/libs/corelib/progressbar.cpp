...
ProgressBar::ProgressBar(QWidget *parent)
    ...
    , m_00mode(false), m_00minimum(0), m_00maximum(100), m_00value(0), m_00signDelta(1)
{
...
void ProgressBar::setRange(int minimum, int maximum)
{
    ...
    if ( !m_minimum && !m_maximum ) {
        m_00mode = true;
        QTimer::singleShot(100, this, SLOT(update_00_value()));
    }
    else {
        m_00mode = false;
    }
...
{
    QSize s;
    if ( m_title == QLatin1String("") )
        s.setWidth(100);
    else
        s.setWidth(200);
    s.setHeight(/* fontMetrics().height() + */PROGRESSBAR_HEIGHT + 0);
    return s;
}
...
void ProgressBar::update_00_value()
{
    m_00value += m_00signDelta;
    if ( m_00value >= m_00maximum || m_00value < 0) {
        m_00signDelta = -m_00signDelta;
        m_00value += m_00signDelta;
    }
    else
        update();
...
void ProgressBar::paintEvent(QPaintEvent *)
{
    ...
    if (bar.isNull())
        bar.load(QLatin1String(":/images/progressbar.png"));
    ...
    p.drawLine(size().width(),0, size().width(), size().height());
    ...
    if ( m_title != "" ) {
        ...
        QRect textRect = rect().adjusted(100, 1, -3, 0);
        textRect.setHeight(h);

        p.setPen(QColor(255, 0, 0, 64));
        p.drawText(textRect, alignment | Qt::AlignVCenter, elidedtitle);
        p.translate(0, -1);
    ...
    QRect rect(INDENT - 1, 2, 86, m_progressHeight-1);
    ...
    if ( !m_00mode ) {
        ...
        int sh = qRound(percent * (inner.width() - 12)) + 1;
        QRect r1(inner.left() + sh, inner.top() + 1, 12, inner.height() - 2);
        p.fillRect(r1, QColor(100, 120, 255));
    }
