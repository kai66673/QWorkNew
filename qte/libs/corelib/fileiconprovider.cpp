...
{
    MimeDatabase *mimeDb = Core::Storage::mainWindow()->mimeDatabase();
    instance()->registerIconOverlayForMimeType(icon, mimeDb->findByType(QString::fromLatin1(mimeType)));
}
...
{
    MimeDatabase *mimeDb = Core::Storage::mainWindow()->mimeDatabase();
    instance()->registerIconOverlayForMimeType(QIcon(QLatin1String(path)), mimeDb->findByType(QString::fromLatin1(mimeType)));
}
