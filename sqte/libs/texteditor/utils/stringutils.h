...
class QTCREATOR_UTILS_EXPORT Hex {
public:
    template <class T> static QString toHexString( const T &a ) {
        QByteArray result;
        {
            QDataStream instraem(&result, QIODevice::WriteOnly);
            instraem << a;
        }

        return QString::fromLocal8Bit(result.toHex());
    }

    template <class T> static T fromHexString( const QString &hexString ) {
        T result;
