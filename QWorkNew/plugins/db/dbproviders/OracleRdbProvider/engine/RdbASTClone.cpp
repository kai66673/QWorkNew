...
CursorNameAST *CursorNameAST::clone(Utils::MemoryPool *pool) const
{
    CursorNameAST *ast = new (pool) CursorNameAST;
    ast->name_token = name_token;
    ast->name = name;
    return ast;
}

StorageMapNameAST *StorageMapNameAST::clone(Utils::MemoryPool *pool) const
{
    StorageMapNameAST *ast = new (pool) StorageMapNameAST;
    ast->name_token = name_token;
    ast->name = name;
    return ast;
}

AreaNameAST *AreaNameAST::clone(Utils::MemoryPool *pool) const
{
    AreaNameAST *ast = new (pool) AreaNameAST;
    ast->name_token = name_token;
    ast->name = name;
    return ast;
}
...
IdentifierNameAST *IdentifierNameAST::clone(Utils::MemoryPool *pool) const
{
    IdentifierNameAST *ast = new (pool) IdentifierNameAST;
...
RoutineNameAST *RoutineNameAST::clone(Utils::MemoryPool *pool) const
{
    RoutineNameAST *ast = new (pool) RoutineNameAST;
    ast->name_token = name_token;
    ast->name = name;
    return ast;
}

ModuleNameAST *ModuleNameAST::clone(Utils::MemoryPool *pool) const
{
    ModuleNameAST *ast = new (pool) ModuleNameAST;
    ast->name_token = name_token;
    ast->name = name;
    return ast;
}
...
IndexTypeAST *IndexTypeAST::clone(Utils::MemoryPool *pool) const
{
    IndexTypeAST *ast = new (pool) IndexTypeAST;
    ast->type_token = type_token;
    ast->is_token = is_token;
    ast->sorted_token = sorted_token;
    ast->ranked_token = ranked_token;
    return ast;
}

IndexNodeSizeAST *IndexNodeSizeAST::clone(Utils::MemoryPool *pool) const
{
    IndexNodeSizeAST *ast = new (pool) IndexNodeSizeAST;
    ast->node_token = node_token;
    ast->size_token = size_token;
    if ( sizeExpr )
        ast->sizeExpr = sizeExpr->clone(pool);
    return ast;
}

IndexStoreInAST *IndexStoreInAST::clone(Utils::MemoryPool *pool) const
{
    IndexStoreInAST *ast = new (pool) IndexStoreInAST;
    ast->store_token = store_token;
    ast->in_token = in_token;
    if ( areaName )
        ast->areaName = areaName->clone(pool);
    return ast;
}

IndexOptionsAST *IndexOptionsAST::clone(Utils::MemoryPool *pool) const
{
    IndexOptionsAST *ast = new (pool) IndexOptionsAST;
    if ( typeAst )
        ast->typeAst = typeAst->clone(pool);
    if ( nodeSizeAst )
        ast->nodeSizeAst = nodeSizeAst->clone(pool);
    if ( storeInAst )
        ast->storeInAst = storeInAst->clone(pool);
    return ast;
}
...
CreateIndexStatementAST *CreateIndexStatementAST::clone(Utils::MemoryPool *pool) const
{
    ...
    ast->rparen_token = rparen_token;
    if ( options )
        ast->options = options->clone(pool);
    return ast;
}
...
CreateStorageMapStatementAST *CreateStorageMapStatementAST::clone(Utils::MemoryPool *pool) const
{
    CreateStorageMapStatementAST *ast = new (pool) CreateStorageMapStatementAST;
    ast->create_token = create_token;
    ast->storage_token = storage_token;
    ast->map_token = map_token;
    if ( mapName )
        ast->mapName = mapName->clone(pool);
    ast->for_token = for_token;
    if ( tableName )
        ast->tableName = tableName->clone(pool);
    ast->store_token = store_token;
    ast->in_token = in_token;
    if ( areaName )
        ast->areaName = areaName->clone(pool);
...
CreateModuleStatementAST *CreateModuleStatementAST::clone(Utils::MemoryPool *pool) const
{
    CreateModuleStatementAST *ast = new (pool) CreateModuleStatementAST;
    ast->create_token = create_token;
    ast->module_token = module_token;
    if ( moduleName )
        ast->moduleName = moduleName->clone(pool);
    ast->language_token = language_token;
    ast->sql_token = sql_token;
    ast->comment_token = comment_token;
    ast->is_token = is_token;
    if ( commentExpr )
        ast->commentExpr = commentExpr->clone(pool);
    for (RoutineListAST *iter = routine_list, **ast_iter = &ast->routine_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) RoutineListAST((iter->value) ? iter->value->clone(pool) : 0);
    ast->end_token = end_token;
    ast->end_module_token = end_module_token;
    return ast;
}

CreateRoutineStatementAST *CreateRoutineStatementAST::clone(Utils::MemoryPool *pool) const
{
    CreateRoutineStatementAST *ast = new (pool) CreateRoutineStatementAST;
    ast->create_token = create_token;
    if ( routine )
        ast->routine = routine->clone(pool);
    return ast;
}

RoutineAST *RoutineAST::clone(Utils::MemoryPool *pool) const
{
    RoutineAST *ast = new (pool) RoutineAST;
    ast->routine_token = routine_token;
    if ( routineName )
        ast->routineName = routineName->clone(pool);
    ast->lparen_token = lparen_token;
    for (ArgumentListAST *iter = arg_list, **ast_iter = &ast->arg_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) ArgumentListAST((iter->value) ? iter->value->clone(pool) : 0);
    ast->rparen_token = rparen_token;
    ast->return_token = return_token;
    if ( returnType )
        ast->returnType = returnType->clone(pool);
    if ( block )
...
DropModuleStatementAST *DropModuleStatementAST::clone(Utils::MemoryPool *pool) const
{
    DropModuleStatementAST *ast = new (pool) DropModuleStatementAST;
    ast->drop_token = drop_token;
    ast->module_token = module_token;
    if ( moduleName )
        ast->moduleName = moduleName->clone(pool);
...
BlockStatementAST *BlockStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockStatementAST *ast = new (pool) BlockStatementAST;
    ast->begin_token = begin_token;
    for (DeclareListAST *iter = decl_list, **ast_iter = &ast->decl_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) DeclareListAST((iter->value) ? iter->value->clone(pool) : 0);
    for (BlockStatementEntryListAST *iter = entry_list, **ast_iter = &ast->entry_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) BlockStatementEntryListAST((iter->value) ? iter->value->clone(pool) : 0);
    ast->end_token = end_token;
    return ast;
}
...
IdentifierExpressionAST *IdentifierExpressionAST::clone(Utils::MemoryPool *pool) const
{
    IdentifierExpressionAST *ast = new (pool) IdentifierExpressionAST;
    if ( ident )
        ast->ident = ident->clone(pool);
    return ast;
}

IdentDotColumnExpressionAST *IdentDotColumnExpressionAST::clone(Utils::MemoryPool *pool) const
{
    IdentDotColumnExpressionAST *ast = new (pool) IdentDotColumnExpressionAST;
    if ( ident )
        ast->ident = ident->clone(pool);
    ast->dot_token = dot_token;
    if ( columnName )
...
DeclareAST *DeclareAST::clone(Utils::MemoryPool *pool) const
{
    DeclareAST *ast = new (pool) DeclareAST;
    ast->declare_token = declare_token;
    if ( identName )
        ast->identName = identName->clone(pool);
    if ( identType )
        ast->identType = identType->clone(pool);
    ast->default_token = default_token;
    if ( defaulExpr )
        ast->defaulExpr = defaulExpr->clone(pool);
    return ast;
}

ArgumentAST *ArgumentAST::clone(Utils::MemoryPool *pool) const
{
    ArgumentAST *ast = new (pool) ArgumentAST;
    ast->inout_spec_token = inout_spec_token;
    if ( ident )
        ast->ident = ident->clone(pool);
    if ( identType )
        ast->identType = identType->clone(pool);
...
BlockInsertStatementAST *BlockInsertStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockInsertStatementAST *ast = new (pool) BlockInsertStatementAST;
    if ( insertStatement )
        ast->insertStatement = insertStatement->clone(pool);
    return ast;
}

BlockUpdateStatementAST *BlockUpdateStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockUpdateStatementAST *ast = new (pool) BlockUpdateStatementAST;
    if ( updateStatement )
        ast->updateStatement = updateStatement->clone(pool);
    return ast;
}

BlockDeleteStatementAST *BlockDeleteStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockDeleteStatementAST *ast = new (pool) BlockDeleteStatementAST;
    if ( deleteStatement )
        ast->deleteStatement = deleteStatement->clone(pool);
    return ast;
}

BlockSelectStatementAST *BlockSelectStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockSelectStatementAST *ast = new (pool) BlockSelectStatementAST;
    ast->select_token = select_token;
    ast->select_spec_token = select_spec_token;
    for (ResultColumnListAST *iter = columnList, **ast_iter = &ast->columnList;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) ResultColumnListAST( (iter->value) ? iter->value->clone(pool) : 0 );
    ast->into_token = into_token;
    for (IdentifierNameListAST *iter = into_list, **ast_iter = &ast->into_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) IdentifierNameListAST( (iter->value) ? iter->value->clone(pool) : 0 );
    ast->from_token = from_token;
    if ( joinSource )
        ast->joinSource = joinSource->clone(pool);
    ast->where_token = where_token;
...
BlockSetStatementAST *BlockSetStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockSetStatementAST *ast = new (pool) BlockSetStatementAST;
    ast->set_token = set_token;
    if ( ident )
        ast->ident = ident->clone(pool);
    ast->equal_token = equal_token;
    if ( assignExpr )
        ast->assignExpr = assignExpr->clone(pool);
    return ast;
}

BlockCommitStatmentAST *BlockCommitStatmentAST::clone(Utils::MemoryPool *pool) const
{
    BlockCommitStatmentAST *ast = new (pool) BlockCommitStatmentAST;
    ast->commit_token = commit_token;
    return ast;
}

BlockRollbackStatmentAST *BlockRollbackStatmentAST::clone(Utils::MemoryPool *pool) const
{
    BlockRollbackStatmentAST *ast = new (pool) BlockRollbackStatmentAST;
    ast->rollback_token = rollback_token;
    return ast;
}

BlockCallStatementAST *BlockCallStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockCallStatementAST *ast = new (pool) BlockCallStatementAST;
    if ( callExpr )
        ast->callExpr = callExpr->clone(pool);
    return ast;
}
...
IfStatementAST *IfStatementAST::clone(Utils::MemoryPool *pool) const
{
    ...
    if ( conditionExpr )
        ast->conditionExpr = conditionExpr->clone(pool);
    ast->then_token = then_token;
    for (BlockStatementEntryListAST *iter = then_entry_list, **ast_iter = &ast->then_entry_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) BlockStatementEntryListAST( (iter->value) ? iter->value->clone(pool) : 0 );
    ast->else_token = else_token;
    for (BlockStatementEntryListAST *iter = else_entry_list, **ast_iter = &ast->else_entry_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) BlockStatementEntryListAST( (iter->value) ? iter->value->clone(pool) : 0 );
    ast->end_token = end_token;
    ast->end_if_token = end_if_token;
    return ast;
}

LoopStatementAST *LoopStatementAST::clone(Utils::MemoryPool *pool) const
{
    LoopStatementAST *ast = new (pool) LoopStatementAST;
    ast->while_token = while_token;
    if ( whileExpr )
        ast->whileExpr = whileExpr->clone(pool);
    ast->loop_token = loop_token;
    for (BlockStatementEntryListAST *iter = entry_list, **ast_iter = &ast->entry_list;
         iter; iter = iter->next, ast_iter = &(*ast_iter)->next)
        *ast_iter = new (pool) BlockStatementEntryListAST( (iter->value) ? iter->value->clone(pool) : 0 );
    ast->end_token = end_token;
    ast->end_loop_token = end_loop_token;
    return ast;
}

DeclareCursorClauseAST *DeclareCursorClauseAST::clone(Utils::MemoryPool *pool) const
{
    DeclareCursorClauseAST *ast = new (pool) DeclareCursorClauseAST;
    ast->read_or_update_token = read_or_update_token;
    ast->only_token = only_token;
    ast->table_token = table_token;
    ast->cursor_token = cursor_token;
    if ( cursorName )
        ast->cursorName = cursorName->clone(pool);
    ast->for_token = for_token;
    return ast;
}
...
ForStatementAST *ForStatementAST::clone(Utils::MemoryPool *pool) const
{
    ...
    ast->for_token = for_token;
    if ( forName )
        ast->forName = forName->clone(pool);
    ast->as_token = as_token;
    ast->each_token = each_token;
    ast->row_token = row_token;
    ast->of_token = of_token;
    if ( cursor )
        ast->cursor = cursor->clone(pool);
    if ( selectStatement )
        ast->selectStatement = selectStatement->clone(pool);
    ...
    for (BlockStatementEntryListAST *iter = entry_list, **ast_iter = &ast->entry_list;
        ...
        *ast_iter = new (pool) BlockStatementEntryListAST( (iter->value) ? iter->value->clone(pool) : 0 );
    ast->end_token = end_token;
    ast->end_for_token = end_for_token;
    return ast;
}

BlockLabelAST *BlockLabelAST::clone(Utils::MemoryPool *pool) const
{
    BlockLabelAST *ast = new (pool) BlockLabelAST;
    if ( labelName )
        ast->labelName = labelName->clone(pool);
    ast->colon_token = colon_token;
    return ast;
}

LeaveStatementAST *LeaveStatementAST::clone(Utils::MemoryPool *pool) const
{
    LeaveStatementAST *ast = new (pool) LeaveStatementAST;
    ast->leave_token = leave_token;
    if ( labelName )
        ast->labelName = labelName->clone(pool);
    return ast;
}

BlockBlockStatementAST *BlockBlockStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockBlockStatementAST *ast = new (pool) BlockBlockStatementAST;
    if ( block )
        ast->block = block->clone(pool);
    return ast;
}

BlockReturnStatementAST *BlockReturnStatementAST::clone(Utils::MemoryPool *pool) const
{
    BlockReturnStatementAST *ast = new (pool) BlockReturnStatementAST;
    ast->return_token = return_token;
    if ( returnExpr )
        ast->returnExpr = returnExpr->clone(pool);
    return ast;
}
