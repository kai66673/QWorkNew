...
////////////////////////////////////////////////////////////////////////////////////////////////////////
// SqlIdentifiersVisitor
SqlIdentifiersVisitor::SqlIdentifiersVisitor( Sql::TranslationUnit *unit, StatementAST *ast, unsigned tokenIndexBefore )
    : ASTVisitor(unit)
    , m_ast(ast)
    , m_editScope(0)
    , m_tokenIndexBefore(tokenIndexBefore)
{
    m_currentScope = &m_globalScope;
}

QStringList SqlIdentifiersVisitor::identifiers() const
{
    QStringList result;
    if ( m_editScope ) {
        m_editScope->fillIdentifiers(result);
        result.removeDuplicates();
    }
    return result;
}

unsigned SqlIdentifiersVisitor::linkTokenIndex( const QString &name )
{
    if ( m_editScope )
        return m_editScope->linkTokenIndex(name);
    return 0;
}

void SqlIdentifiersVisitor::endVisit( RoutineAST *ast )
{
    if ( ast )
        m_currentScope = m_currentScope->parent();
}

bool SqlIdentifiersVisitor::visit( ArgumentAST *ast )
{
    if ( ast && ast->ident )
        m_currentScope->addIdentifier(ast->ident);
    return false;
}

bool SqlIdentifiersVisitor::visit( RoutineAST *ast )
{
    if ( ast ) {
        m_currentScope = m_currentScope->createChildScope();
        if ( m_tokenIndexBefore >= ast->firstToken() && m_tokenIndexBefore <= ast->lastToken() )
            m_editScope = m_currentScope;
        accept(ast->arg_list);
        accept(ast->block);
    }
    return false;
...
bool SqlIdentifiersVisitor::visit( DeclareAST *ast )
{
    if ( ast && ast->identName )
        m_currentScope->addIdentifier(ast->identName);
    return false;
}

void SqlIdentifiersVisitor::endVisit( BlockStatementAST *ast )
{
    if ( ast )
        m_currentScope = m_currentScope->parent();
}

bool SqlIdentifiersVisitor::visit( BlockStatementAST *ast )
{
    if ( ast ) {
        m_currentScope = m_currentScope->createChildScope();
        if ( m_tokenIndexBefore >= ast->firstToken() && m_tokenIndexBefore <= ast->lastToken() )
            m_editScope = m_currentScope;
        accept(ast->decl_list);
        accept(ast->entry_list);
    }
    return false;
}
