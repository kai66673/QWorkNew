...
bool ASTMatcher::match( IdentifierNameAST *node, IdentifierNameAST *pattern )
{
...
bool RDB::ASTMatcher::match( ModuleNameAST *node, ModuleNameAST *pattern )
{
    pattern->name_token = node->name_token;
    return true;
}

bool ASTMatcher::match( RoutineNameAST *node, RoutineNameAST *pattern )
{
    pattern->name_token = node->name_token;
    return true;
}
...
bool ASTMatcher::match( CursorNameAST *node, CursorNameAST *pattern )
{
    pattern->name_token = node->name_token;
    return true;
}

bool ASTMatcher::match( StorageMapNameAST *node, StorageMapNameAST *pattern )
{
    pattern->name_token = node->name_token;
    return true;
}

bool ASTMatcher::match( AreaNameAST *node, AreaNameAST *pattern )
{
    pattern->name_token = node->name_token;
    return true;
}
...
bool ASTMatcher::match( IndexTypeAST *node, IndexTypeAST *pattern )
{
    pattern->type_token = node->type_token;
    pattern->is_token = node->is_token;
    pattern->sorted_token = node->sorted_token;
    pattern->ranked_token = node->ranked_token;

    return true;
}

bool ASTMatcher::match( IndexNodeSizeAST *node, IndexNodeSizeAST *pattern )
{
    pattern->node_token = node->node_token;
    pattern->size_token = node->size_token;

    if ( !pattern->sizeExpr )
        pattern->sizeExpr = node->sizeExpr;
    else if ( !AST::match(node->sizeExpr, pattern->sizeExpr, this) )
        return false;

    return true;
}

bool ASTMatcher::match( IndexStoreInAST *node, IndexStoreInAST *pattern )
{
    pattern->store_token = node->store_token;
    pattern->in_token = node->in_token;

    if ( !pattern->areaName )
        pattern->areaName = node->areaName;
    else if ( !AST::match(node->areaName, pattern->areaName, this) )
        return false;

    return true;
}

bool ASTMatcher::match( IndexOptionsAST *node, IndexOptionsAST *pattern )
{
    if ( !pattern->typeAst )
        pattern->typeAst = node->typeAst;
    else if ( !AST::match(node->typeAst, pattern->typeAst, this) )
        return false;

    if ( !pattern->nodeSizeAst )
        pattern->nodeSizeAst = node->nodeSizeAst;
    else if ( !AST::match(node->nodeSizeAst, pattern->nodeSizeAst, this) )
        return false;

    if ( !pattern->storeInAst )
        pattern->storeInAst = node->storeInAst;
    else if ( !AST::match(node->storeInAst, pattern->storeInAst, this) )
        return false;
...
bool ASTMatcher::match( CreateIndexStatementAST *node, CreateIndexStatementAST *pattern )
{
    ...
    if ( !pattern->options )
        pattern->options = node->options;
    else if ( !AST::match(node->options, pattern->options, this) )
        return false;
...
bool ASTMatcher::match( CreateStorageMapStatementAST *node, CreateStorageMapStatementAST *pattern )
{
    pattern->create_token = node->create_token;
    pattern->storage_token = node->storage_token;
    pattern->map_token = node->map_token;

    if ( !pattern->mapName )
        pattern->mapName = node->mapName;
    else if ( !AST::match(node->mapName, pattern->mapName, this) )
        return false;
...
bool RDB::ASTMatcher::match( CreateModuleStatementAST *node, CreateModuleStatementAST *pattern )
{
    pattern->create_token = node->create_token;
    pattern->module_token = node->module_token;

    if ( !pattern->moduleName )
        pattern->moduleName = node->moduleName;
    else if ( !AST::match(node->moduleName, pattern->moduleName, this) )
        return false;

    pattern->language_token = node->language_token;
    pattern->sql_token = node->sql_token;
    pattern->comment_token = node->comment_token;
    pattern->is_token = node->is_token;

    if ( !pattern->commentExpr )
        pattern->commentExpr = node->commentExpr;
    else if ( !AST::match(node->commentExpr, pattern->commentExpr, this) )
        return false;

    if ( !pattern->routine_list )
        pattern->routine_list = node->routine_list;
    else if ( !AST::match(node->routine_list, pattern->routine_list, this) )
        return false;

    pattern->end_token = node->end_token;
    pattern->end_module_token = node->end_module_token;

    return true;
}

bool RDB::ASTMatcher::match( CreateRoutineStatementAST *node, CreateRoutineStatementAST *pattern )
{
    pattern->create_token = node->create_token;

    if ( !pattern->routine )
        pattern->routine = node->routine;
    else if ( !AST::match(node->routine, pattern->routine, this) )
        return false;

    return true;
}

bool ASTMatcher::match( RoutineAST *node, RoutineAST *pattern )
{
    pattern->routine_token = node->routine_token;

    if ( !pattern->routineName )
        pattern->routineName = node->routineName;
    else if ( !AST::match(node->routineName, pattern->routineName, this) )
        return false;
    ...
    if ( !pattern->returnType )
        pattern->returnType = node->returnType;
    else if ( !AST::match(node->returnType, pattern->returnType, this) )
        return false;
...
bool ASTMatcher::match( DropModuleStatementAST *node, DropModuleStatementAST *pattern )
{
    pattern->drop_token = node->drop_token;
    pattern->module_token = node->module_token;
...
bool ASTMatcher::match( BlockStatementAST *node, BlockStatementAST *pattern )
{
    pattern->begin_token = node->begin_token;

    if ( !pattern->decl_list )
        pattern->decl_list = node->decl_list;
    else if ( !AST::match(node->decl_list, pattern->decl_list, this) )
        return false;

    if ( !pattern->entry_list )
        pattern->entry_list = node->entry_list;
    else if ( !AST::match(node->entry_list, pattern->entry_list, this) )
        return false;
...
bool ASTMatcher::match( IdentifierExpressionAST *node, IdentifierExpressionAST *pattern )
{
    if ( !pattern->ident )
        pattern->ident = node->ident;
    else if ( !AST::match(node->ident, pattern->ident, this) )
        return false;

    return true;
}

bool ASTMatcher::match( IdentDotColumnExpressionAST *node, IdentDotColumnExpressionAST *pattern )
{
    if ( !pattern->ident )
        pattern->ident = node->ident;
    else if ( !AST::match(node->ident, pattern->ident, this) )
...
bool ASTMatcher::match( DeclareAST *node, DeclareAST *pattern )
{
    pattern->declare_token = node->declare_token;

    if ( !pattern->identName )
        pattern->identName = node->identName;
    else if ( !AST::match(node->identName, pattern->identName, this) )
        return false;

    if ( !pattern->identType )
        pattern->identType = node->identType;
    else if ( !AST::match(node->identType, pattern->identType, this) )
        return false;

    pattern->default_token = node->default_token;

    if ( !pattern->defaulExpr )
        pattern->defaulExpr = node->defaulExpr;
    else if ( !AST::match(node->defaulExpr, pattern->defaulExpr, this) )
        return false;

    return true;
}

bool ASTMatcher::match( ArgumentAST *node, ArgumentAST *pattern )
{
    pattern->inout_spec_token = node->inout_spec_token;
...
bool ASTMatcher::match( BlockInsertStatementAST *node, BlockInsertStatementAST *pattern )
{
    if ( !pattern->insertStatement )
        pattern->insertStatement = node->insertStatement;
    else if ( !AST::match(node->insertStatement, pattern->insertStatement, this) )
        return false;

    return true;
}

bool ASTMatcher::match( BlockUpdateStatementAST *node, BlockUpdateStatementAST *pattern )
{
    if ( !pattern->updateStatement )
        pattern->updateStatement = node->updateStatement;
    else if ( !AST::match(node->updateStatement, pattern->updateStatement, this) )
        return false;

    return true;
}

bool ASTMatcher::match( BlockDeleteStatementAST *node, BlockDeleteStatementAST *pattern )
{
    if ( !pattern->deleteStatement )
        pattern->deleteStatement = node->deleteStatement;
    else if ( !AST::match(node->deleteStatement, pattern->deleteStatement, this) )
        return false;

    return true;
}

bool ASTMatcher::match( BlockSelectStatementAST *node, BlockSelectStatementAST *pattern )
{
    pattern->select_token = node->select_token;
    pattern->select_spec_token = node->select_spec_token;
    ...
    if ( !pattern->into_list )
        pattern->into_list = node->into_list;
    else if ( !AST::match(node->into_list, pattern->into_list, this) )
        return false;
...
bool ASTMatcher::match( BlockSetStatementAST *node, BlockSetStatementAST *pattern )
{
    pattern->set_token = node->set_token;
    ...
    if ( !pattern->assignExpr )
        pattern->assignExpr = node->assignExpr;
    else if ( !AST::match(node->assignExpr, pattern->assignExpr, this) )
        return false;

    return true;
}

bool ASTMatcher::match( BlockCommitStatmentAST *node, BlockCommitStatmentAST *pattern )
{
    pattern->commit_token = node->commit_token;
...
bool ASTMatcher::match( BlockRollbackStatmentAST *node, BlockRollbackStatmentAST *pattern )
{
    pattern->rollback_token = node->rollback_token;
...
bool ASTMatcher::match( BlockCallStatementAST *node, BlockCallStatementAST *pattern )
{
    if ( !pattern->callExpr )
        pattern->callExpr = node->callExpr;
    else if ( !AST::match(node->callExpr, pattern->callExpr, this) )
        return false;
...
bool ASTMatcher::match( IfStatementAST *node, IfStatementAST *pattern )
{
    ...
    if ( !pattern->conditionExpr )
        pattern->conditionExpr= node->conditionExpr;
    else if ( !AST::match(node->conditionExpr, pattern->conditionExpr, this) )
        return false;

    pattern->then_token = node->then_token;

    if ( !pattern->then_entry_list )
        pattern->then_entry_list = node->then_entry_list;
    else if ( !AST::match(node->then_entry_list, pattern->then_entry_list, this) )
        return false;

    pattern->else_token = node->else_token;

    if ( !pattern->else_entry_list )
        pattern->else_entry_list = node->else_entry_list;
    else if ( !AST::match(node->else_entry_list, pattern->else_entry_list, this) )
        return false;

    pattern->end_token = node->end_token;
    pattern->end_if_token = node->end_if_token;

    return true;
}

bool ASTMatcher::match( LoopStatementAST *node, LoopStatementAST *pattern )
{
    pattern->while_token = node->while_token;

    if ( !pattern->whileExpr )
        pattern->whileExpr = node->whileExpr;
    else if ( !AST::match(node->whileExpr, pattern->whileExpr, this) )
        return false;

    pattern->loop_token = node->loop_token;
    ...
    pattern->end_token = node->end_token;
    pattern->end_loop_token = node->end_loop_token;

    return true;
}

bool ASTMatcher::match( DeclareCursorClauseAST *node, DeclareCursorClauseAST *pattern )
{
    pattern->read_or_update_token = node->read_or_update_token;
    pattern->only_token = node->only_token;
    pattern->table_token = node->table_token;
    pattern->cursor_token = node->cursor_token;

    if ( !pattern->cursorName )
        pattern->cursorName = node->cursorName;
    else if ( !AST::match(node->cursorName, pattern->cursorName, this) )
        return false;
...
bool ASTMatcher::match( ForStatementAST *node, ForStatementAST *pattern )
{
    ...
    if ( !pattern->forName )
        pattern->forName = node->forName;
    else if ( !AST::match(node->forName, pattern->forName, this) )
        return false;
    ...
    if ( !pattern->cursor )
        pattern->cursor = node->cursor;
    else if ( !AST::match(node->cursor, pattern->cursor, this) )
        return false;
    ...
    pattern->end_token = node->end_token;
    pattern->end_for_token = node->end_for_token;

    return true;
}

bool ASTMatcher::match( BlockLabelAST *node, BlockLabelAST *pattern )
{
    if ( !pattern->labelName )
        pattern->labelName = node->labelName;
    else if ( !AST::match(node->labelName, pattern->labelName, this) )
        return false;
...
bool ASTMatcher::match( LeaveStatementAST *node, LeaveStatementAST *pattern )
{
    pattern->leave_token = node->leave_token;
...
bool ASTMatcher::match( BlockBlockStatementAST *node, BlockBlockStatementAST *pattern )
{
    if ( !pattern->block )
        pattern->block = node->block;
    else if ( !AST::match(node->block, pattern->block, this) )
...
bool RDB::ASTMatcher::match( BlockReturnStatementAST *node, BlockReturnStatementAST *pattern )
{
    pattern->return_token = node->return_token;

    if ( !pattern->returnExpr )
        pattern->returnExpr = node->returnExpr;
    else if ( !AST::match(node->returnExpr, pattern->returnExpr, this) )
        return false;
