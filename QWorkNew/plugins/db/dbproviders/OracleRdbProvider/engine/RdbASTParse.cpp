...
{
    if ( p->skipUntil(Sql::T_SEMICOLON) )
        p->consumeToken();
}

void CreateRoutineStatementAST::skip( Parser *p )
{
    if ( routine )
        routine->skip(p);
    else if ( p->skipUntil(Sql::T_SEMICOLON) )
        p->consumeToken();
}

void RoutineAST::skip( Parser *p )
{
    if ( block ) {
        block->skip(p);
    }
...
bool IndexOptionsAST::parse( Parser *p, Utils::MemoryPool *pool )
{
    if ( p->LA() == Sql::T_SEMICOLON || p->LA() == Sql::T_EOF_SYMBOL )
        return true;

    if ( p->LA1() == T_4_TYPE ) {
        typeAst = new (pool) IndexTypeAST;
        typeAst->type_token = p->consumeToken();
        if ( p->LA1() == T_2_IS ) {
            typeAst->is_token = p->consumeToken();
            if ( p->LA1() == T_6_SORTED ) {
                typeAst->sorted_token = p->consumeToken();
                if ( p->LA1() == T_6_RANKED )
                    typeAst->ranked_token = p->consumeToken();
            }
            else
                p->errorCursor("Expected Toke `SORTED'");
        }
        else
            p->errorCursor("Expected Token `IS'");
    }

    if ( p->LA1() == T_4_NODE ) {
        nodeSizeAst = new (pool) IndexNodeSizeAST;
        nodeSizeAst->node_token = p->consumeToken();
        if ( p->LA1() == T_4_SIZE ) {
            nodeSizeAst->size_token = p->consumeToken();
            if ( p->LA() == Sql::T_NUMERIC_LITERAL ) {
                nodeSizeAst->sizeExpr = new (pool) NumericExpressionAST;
                nodeSizeAst->sizeExpr->numeric_token = p->consumeToken();
            }
            else
                p->errorCursor("Expected Numeric Expression");
        }
        else
            p->errorCursor("Expected Token `SIZE'");
    }

    if ( p->LA1() == T_5_STORE ) {
        storeInAst = new (pool) IndexStoreInAST;
        storeInAst->store_token = p->consumeToken();
        if ( p->LA1() == T_2_IN ) {
            storeInAst->in_token = p->consumeToken();
            if ( p->LA() == Sql::T_IDENTIFIER ) {
                storeInAst->areaName = new (pool) AreaNameAST;
                const Sql::Token &tk = p->tok();
                storeInAst->areaName->name = tk.identifier;
                storeInAst->areaName->name_token = p->consumeToken();
            }
            else
                p->errorCursor("Area Name Expected");
        }
        else
            p->errorCursor("Expected Token `IN'");
    }

    if ( p->LA() != Sql::T_SEMICOLON && p->LA() != Sql::T_EOF_SYMBOL )
        p->errorCursor("Unexpected Token");
...
{
    if ( alterTableClause && alterTableClause->objectKind == AlterTableClauseAST::Constraint )
...
bool AlterTableStatementAST::targetTableObjectName( QString *tableName, unsigned *tableObjectType ) const
{
    ...
    *tableName = schemaTableName->tableName->name->chars();
    switch ( alterTableClause->objectKind ) {
        case AlterTableClauseAST::Unknown:     return false;
        case AlterTableClauseAST::Constraint:  *tableObjectType = Sql::Constants::DbSchemaObjectSubtype::C_CONSTRAINT; break;
        case AlterTableClauseAST::Field:       *tableObjectType = Sql::Constants::DbSchemaObjectSubtype::C_COLUMN; break;
    }
...
bool AlterTableAddColumnClauseAST::parse( Parser *p, Utils::MemoryPool *pool )
{
    ...
    if ( p->isPatentialIdentifier() ) {
        ...
        if ( p->LA1() == T_7_DEFAULT ) {
            ...
            else if ( p->LA() == Sql::T_NUMERIC_LITERAL ) {
            ...
            else if ( p->LA() == Sql::T_CHAR_LITERAL ) {
...
bool SchemaIndexNameAST::parse( Parser *p, Utils::MemoryPool *pool )
{
    ...
    if ( p->LA(2) == Sql::T_DOT && p->isPatentialIdentifier(3) ) {
        const Sql::Token &stk = p->tok();
        schemaName = new (pool) SchemaNameAST;
        schemaName->name = stk.identifier;
        schemaName->name_token = p->consumeToken();
...
void BlockStatementAST::skip( Parser *p )
{
    while ( p->LA() ) {
        if ( p->LA1() == T_3_END && p->LA(2) == Sql::T_SEMICOLON ) {
            end_token = p->consumeToken();
            p->consumeToken();
            return;
        }
        p->consumeToken();
...
void CreateModuleStatementAST::skip( Parser *p )
{
    while ( p->LA() ) {
        if ( p->LA1() == T_3_END && p->LA1(2) == T_6_MODULE && p->LA(3) == Sql::T_SEMICOLON ) {
            end_token = p->consumeToken();
            end_module_token = p->consumeToken();
            p->consumeToken();
            return;
        }
        p->consumeToken();
