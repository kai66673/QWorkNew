...
#include "Core.h"
#include "QLalrOracleProvider.h"
#include "QLalrOracleParserEngine.h"
#include "QLalrOracleAuthWidget.h"
#include "DbUtils.h"
#include "qlalr_oracle_sql_constants.h"
...
Provider::Provider( QObject *parent )
    : QObject(parent)
    , m_engine(new ::QLalrOracle::OracleParserEngine())
{
}
...
BaseConnectionInfo *Provider::createConnection( const QString &connName, const QStringList &parameters ) const
{
    ...
    if ( !db.open() ) {
        QMessageBox::critical( Core::Storage::mainWindow()->widget(),
                               ...
                               QMessageBox::Cancel );
        Core::LogManager::addMessage( Core::LogMessage(tr("Error On Connecting to DB: %1").arg(db.lastError().text()), "Error") );
        return 0;
    }
...
{
    return new QLalrOracleAuthWidget(parent);
}
        ...
        case Sql::Constants::DbSchemaObjectType::C_TABLE:
            switch ( schemaObjectSubtype ) {
                ...
                case Sql::Constants::DbSchemaObjectSubtype::C_TABLE_INDEX:
                    return MetadataQuery::create("select index_name from all_indexes where owner = \'" + schemaName.toUpper() + "\' and table_name = \'" + objName.toUpper() +"\' order by index_name");
                case Sql::Constants::DbSchemaObjectSubtype::C_TABLE_TRIGGER:
                    return MetadataQuery::create("select trigger_name from all_triggers where owner = \'" + schemaName.toUpper() + "\' and table_name = \'" + objName.toUpper() + "\' order by trigger_name");
                case Sql::Constants::DbSchemaObjectSubtype::C_TABLE_CONSTRAINT:
                    return MetadataQuery::create("select constraint_name from all_constraints where owner = \'" + schemaName.toUpper() + "\' and table_name = \'" + objName.toUpper() + "\' order by constraint_name");
                default:
                    return MetadataQuery::Ptr(0);
            }
