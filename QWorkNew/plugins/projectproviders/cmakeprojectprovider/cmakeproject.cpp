...
CMakeProject::~CMakeProject()
{
    ...
    /// NOTE:! already deleted by SessionNode dtor
    // delete m_rootNode;
}
...
void CMakeProject::changeActiveBuildConfiguration(ProjectExplorer::BuildConfiguration *bc)
{
    ...
    if (mode != CMakeOpenProjectWizard::Nothing) {
        ...
        CMakeOpenProjectWizard copw(Core::Storage::mainWindow()->widget(), m_manager, mode, &info);
...
QString CMakeProject::filePath() const
{
    return m_fileName;
}
...
bool CMakeProject::fromMap(const QVariantMap &map)
{
    ...
    if (!hasUserFile) {
        CMakeOpenProjectWizard copw(Core::Storage::mainWindow()->widget(), m_manager, projectDirectory().toString(), Utils::Environment::systemEnvironment());
        if (copw.exec() != QDialog::Accepted)
            return false;
        Kit *k = copw.kit();
...
CMakeFile::CMakeFile(CMakeProject *parent, QString fileName, IDocumentPlugin *documentPlugin)
    : PlainTextDocument(documentPlugin)
    , m_project(parent)
{
    /// TODO: ?
...
PlainTextDocument::ReloadBehavior CMakeFile::reloadBehavior(ChangeTrigger state, ChangeType type) const
{
    Q_UNUSED(state)
    Q_UNUSED(type)
