...
//        if (QMessageBox::question(Core::Storage::mainWindow()->widget(), title, message, QMessageBox::Yes | QMessageBox::No) == QMessageBox::Yes) {
//            QString error;
...
class QmakeProjectFile : public PlainTextDocument
{
    Q_OBJECT
...
PlainTextDocument::ReloadBehavior QmakeProjectFile::reloadBehavior(ChangeTrigger state, ChangeType type) const
{
    Q_UNUSED(state)
    Q_UNUSED(type)
...
//    m_rootProjectNode = new QmakeProFileNode(this, m_fileInfo->filePath(), this);
    m_rootProjectNode = new QmakeProFileNode(this, m_projectFilePath, this);
...
bool QmakeProject::equalFileList(const QStringList &a, const QStringList &b)
{
    ...
    while (ait != aend && bit != bend) {
        if (*ait == CppTools::Internal::CppModelManager::configurationFileName())
            ++ait;
        else if (*bit == CppTools::Internal::CppModelManager::configurationFileName())
            ++bit;
        else if (*ait == *bit)
            ++ait, ++bit;
        else
...
void QmakeProject::updateCppCodeModel()
{
    ...
    foreach (QmakeProFileNode *pro, proFiles) {
        ...
        { // C++ files:
            ...
            cppPart->files.prepend(ProjectFile(CppTools::Internal::CppModelManager::configurationFileName(),
...
void QmakeProject::asyncUpdate()
{
    ...
    Core::Storage::mainWindow()->progressManager()->addTask(m_asyncUpdateFutureInterface->future(),
...
IProjectProvider *QmakeProject::projectManager() const
{
    return m_manager;
}
...
QString QmakeProject::filePath() const
{
...
{
    ProjectManager::instance()->reportError(errorMessage);
}
...
//        QString dir = QFileInfo(m_fileInfo->filePath()).absolutePath();
        QString dir = QFileInfo(m_projectFilePath).absolutePath();
...
void QmakeProject::configureAsExampleProject(const QStringList &platforms)
{
    ...
    qDeleteAll(infoList);
    /// TODO:
//    ProjectManager::requestProjectModeUpdate(this);
}
