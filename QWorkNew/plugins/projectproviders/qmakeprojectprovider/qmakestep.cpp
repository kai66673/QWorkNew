...
{
    QMessageBox *question = new QMessageBox(Core::Storage::mainWindow()->widget());
...
void QMakeStepConfigWidget::recompileMessageBoxFinished(int button)
{
    if (button == QMessageBox::Yes) {
        ...
        BuildManager::buildLists(stepLists, QStringList() << ProjectManager::displayNameForStepId(clean)
                       << ProjectManager::displayNameForStepId(build));
    }
}
