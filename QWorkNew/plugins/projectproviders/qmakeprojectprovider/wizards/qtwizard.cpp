...
{
    QString rc = qApp->applicationDirPath();
...
bool QtWizard::lowerCaseFiles()
{
    ...
    return ProjectManager::settings()->value(lowerCaseSettingsKey, QVariant(lowerCaseDefault)).toBool();
}
...
bool BaseQmakeProjectWizardDialog::writeUserFile(const QString &proFileName) const
{
    ...
    QmakeManager *manager = QmakeManager::instance();
    Q_ASSERT(manager);
