...
#include <cpp/formclasswizardparameters.h>
                                     ...
                                     QString *errorMessage)
{
    ...
    Designer::QtDesignerFormClassCodeGenerator::generateCpp(fp, &headerContents, &sourceContents);
                                                 ...
                                                 QString *errorMessage) const
{
    ...
    mainSource.setContents(ProjectExplorer::ProjectManager::instance()->licenseTemplate(mainSourceFileName)
                           + contents);
    ...
    if (params.designerForm) {
        ...
        if (!parametrizeTemplate(templatePath, formSourceTemplate, params, &contents, errorMessage))
            return Core::GeneratedFiles();
        formSource.setContents(ProjectExplorer::ProjectManager::instance()->licenseTemplate(formSourceFileName)
                               + contents);
        ...
        if (!parametrizeTemplate(templatePath, formHeaderTemplate, params, &contents, errorMessage))
            return Core::GeneratedFiles();
        formHeader.setContents(ProjectExplorer::ProjectManager::instance()->licenseTemplate(formHeaderName)
                               + contents);
    }
