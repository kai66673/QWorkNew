...
void Lexer::scan_helper( Token *tok )
{
    ...
    switch (ch) {
        case '"':
        {
            ...
            if ( _translationUnit )
                tok->identifier = _translationUnit->control()->identifier(yytext + 1, yylen - 2);
        } break;
        case '\'':
        {
            tok->f.kind = T_STRING_LITERAL;
        ...
        default:
            if ( std::isalpha(ch) || ch == '_' || ch == '$' || ch == '%' ) {
                const char *yytext = _currentChar - 1;
                while ( std::isalnum(_yychar) || _yychar == '_' || _yychar == '$' || _yychar == '%' )
                    yyinp();
                int yylen = _currentChar - yytext;
                if ( f._scanKeywords )
                    Keywords::classify(yytext, yylen, *tok);
                tok->f.kind = T_IDENTIFIER;
                if ( tok->isKeyword() )
                    tok->f.kind = T_KEYWORD;
                if ( _translationUnit )
                    tok->identifier = _translationUnit->control()->identifier(yytext, yylen);
