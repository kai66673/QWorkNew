...
{
    QString lowerCaseSettingsKey = QLatin1String(ProjectExplorer::Constants::CPPTOOLS_SETTINGSGROUP);
    lowerCaseSettingsKey += QLatin1Char('/');
    lowerCaseSettingsKey += QLatin1String(ProjectExplorer::Constants::LOWERCASE_CPPFILES_KEY);
    const bool lowerCaseDefault = false;
    return ProjectExplorer::ProjectManager::settings()->value(lowerCaseSettingsKey, QVariant(lowerCaseDefault)).toBool();
}
...
void FormClassWizardPage::initFileGenerationSettings()
{
    ...
    m_ui->newClassWidget->setHeaderExtension(mimeDb->preferredSuffixByType(QLatin1String(ProjectExplorer::Constants::CPP_HEADER_MIMETYPE)));
    m_ui->newClassWidget->setSourceExtension(mimeDb->preferredSuffixByType(QLatin1String(ProjectExplorer::Constants::CPP_SOURCE_MIMETYPE)));
