...
SemanticInfo SemanticHighlighter::semanticInfo(const SemanticHighlighter::Source &source)
{
    ...
    IParserEngine *parserEngine = m_lastSemanticInfo.m_parserEngine;
    m_mutex.unlock();

    Document::Ptr doc;
    QList<Utils::DiagnosticMessage> diagnosticMessages;

    if ( !source.force && revision == source.revision && parserEngine == source.parserEngine ) {
        m_mutex.lock();
    ...
    if ( !doc ) {
        ...
        doc->parseDocument(ParseSqlNames, source.parserEngine);
    }
    ...
    semanticInfo.m_parserEngine = source.parserEngine;
