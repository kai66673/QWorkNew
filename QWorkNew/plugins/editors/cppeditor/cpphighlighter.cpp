...
CppHighlighter::CppHighlighter(QTextDocument *document)
    ...
    , m_singleLineCommentEnabled(true)
    , m_extraExecSqlEnabled(false)
{
}
...
void CppHighlighter::highlightBlock(const QString &text)
{
    ...
    tokenize.setSingleLineCommentEnabled(m_singleLineCommentEnabled);
    tokenize.setExtraExecSqlEnabled(m_extraExecSqlEnabled);

    int initialState = state;
    const QList<Token> tokens = tokenize(text, initialState, sqlState);
    state = tokenize.state(); // refresh the state
    sqlState = tokenize.sqlState();
    ...
    for (int i = 0; i < tokens.size(); ++i) {
        ...
        if (i == 0 && tk.is(T_POUND)) {
            ...
            expectPreprocessorKeyword = true;
                   ...
                   (tk.isKeyword() || tk.isExecSqlKeyword() || tk.is(T_IDENTIFIER)) && isPPKeyword(text.midRef(tk.begin(), tk.length()))) {
        ...
        else if (tk.is(T_EXEC_SQL_INCLUDE_FILENAME))
            ...
            setFormat(tk.begin(), tk.length(), m_formats[SqlKeywordFormat]);
    ...
    if (currentState != -1) {
        int oldState = currentState & 0x0f;
    ...
    setCurrentBlockState((braceDepth << 8) | (tokenize.sqlState() << 4) | tokenize.state());
}
...
void CppHighlighter::onFormatSettingsChanged( bool forceRehilight )
{
